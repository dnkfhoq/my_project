...
PerceptorHandler::OnLink()
{
    ...
    std::shared_ptr<Transform> transformParent = std::static_pointer_cast<Transform>
  ...
  //std::shared_ptr<CollisionPerceptor> perceptor =
  //  std::static_pointer_cast<CollisionPerceptor>
  //  (transformParent->GetChildOfClass("CollisionPerceptor", true));

  mColPercept = std::dynamic_pointer_cast<CollisionPerceptor>
...
(std::shared_ptr<Collider> collidee, GenericContact& /*contact*/)
{
    if (mColPercept.get() == 0)
        return;

  // now find the closest Transform node above the collidee
  std::shared_ptr<Transform> colTransformParent = std::static_pointer_cast<Transform>
