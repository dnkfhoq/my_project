...
void ForceEffector::PrePhysicsUpdateInternal(float /*deltaTime*/)
{
  ...
  std::shared_ptr<ForceAction> forceAction =
    std::dynamic_pointer_cast<ForceAction>(mAction);
    mAction.reset();
...
ForceEffector::GetActionObject(const Predicate& predicate)
{
  ...
  return std::shared_ptr<ActionObject>(new ForceAction(GetPredicate(),force));
}
