...
{
  mRender = std::dynamic_pointer_cast<ImageRender > (GetCore()->New("ImageRender"));

  mCamera = std::dynamic_pointer_cast<Camera > (GetCore()->New("oxygen/Camera"));
  if (0 != mCamera.get())
  {
...
bool ImagePerceptor::Percept(std::shared_ptr<PredicateList> predList)
{
