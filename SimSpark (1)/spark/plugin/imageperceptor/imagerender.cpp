...
void ImageRender::SetCamera(std::shared_ptr<oxygen::Camera> camera)
{
  mCamera = camera;
}
...
void ImageRender::Render()
{
  ...
  if (mOffScreen)
  {
    ...
    // setup the camera
    std::shared_ptr<Camera> oldCamera = mRenderServer->GetCamera();
  ...
  if (!mOffScreen)
  {
    ...
    if ( size != mDataSize )
    {
      mData = std::shared_ptr<char[]>(new char[size]);
      mDataSize = size;
    }
