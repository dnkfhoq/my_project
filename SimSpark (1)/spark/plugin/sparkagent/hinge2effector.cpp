...
bool Hinge2Effector::Realize(std::shared_ptr<ActionObject> action)
{
    if (mJoint.get() == 0)
        {
            return false;
    ...
    std::shared_ptr<Hinge2Action> hinge2Action =
        std::dynamic_pointer_cast<Hinge2Action>(action);
...
std::shared_ptr<ActionObject> Hinge2Effector::GetActionObject(const Predicate& predicate)
{
    for(;;)
        {
            if (mJoint.get() == 0)
            ...
            return std::shared_ptr<Hinge2Action>(new Hinge2Action(GetPredicate(),velocity));
        }
