...
std::shared_ptr<TriMesh> ObjImporter::ImportMesh
(const string& name, const ParameterList& parameter)
{
    ...
    std::shared_ptr<float[]> posArray(new float[totalFaceCount * 9]);

    // for one face: 3 x <u,v,0.0> (last value ignored)
    std::shared_ptr<float[]> texArray(new float[totalFaceCount * 9]);

    // for one face: 3 x <x,y,z>
    std::shared_ptr<float[]> normArray(new float[totalFaceCount * 9]);
    ...
    for (
         ...
         )
        {
            ...
            std::shared_ptr<IndexBuffer> indeces(new IndexBuffer());
            // 3 indices per face
...
bool ObjImporter::SetupMaterials(string & matLibName, TObjMatValueVector & matVector)
{
    ...
    RGBA rgba;
    std::shared_ptr<MaterialSolid> material;
    ...
    for (
         ...
         )
        {
            ...
            if (iter->diffuseTexture.size())
                {
                    material = std::dynamic_pointer_cast<Material2DTexture>
                ...
                {
                    material = std::dynamic_pointer_cast<MaterialSolid>
