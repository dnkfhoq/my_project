...
std::unique_ptr<GenericMass, GenericMassDeleter> RigidBodyImp::CreateMass(float mass, salt::Vector3f cVector)
{
    dMass* ODEMass = new dMass;
    ODEMass->mass = mass;
    ODEMass->c[0] = cVector[0];
    ODEMass->c[1] = cVector[1];
    ODEMass->c[2] = cVector[2];
    
    GenericMass* massPtr = reinterpret_cast<GenericMass*>(ODEMass);
    return { massPtr, dMassDeleter() };
}
...
void RigidBodyImp::dMassDeleter::operator()(GenericMass* mass)
{
    delete reinterpret_cast<dMass*>(mass);
}
