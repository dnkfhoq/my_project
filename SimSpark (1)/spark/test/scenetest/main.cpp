...
std::shared_ptr<kerosin::Font>        gFont;
...
static bool init()
{
        ...
        // load font
        std::shared_ptr<FontServer> fontServer = std::static_pointer_cast<FontServer>(gContext->Get("/sys/server/font"));
...
static float processInput()
{
        ...
        std::shared_ptr<FPSController> fpsController = std::static_pointer_cast<FPSController>(gContext->Get("/usr/scene1/camera0/_body/fps"));
...
static void renderFrame(float deltaTime)
{
        ...
        shared_ptr<SceneServer> sceneServer = std::static_pointer_cast<SceneServer>(gContext->Get("/sys/server/scene"));
        if (sceneServer.get() != NULL)
        {
                sceneServer->Render();
        }
#else
    std::shared_ptr<RenderServer> renderServer =
        std::static_pointer_cast<RenderServer>(gContext->Get("/sys/server/scene"));
    if (renderServer)
    {
...
static void update()
{
        ...
        std::shared_ptr<SceneServer> sceneServer = std::static_pointer_cast<SceneServer>(gContext->Get("/sys/server/scene"));
        if (sceneServer.get() != NULL)
        {
