...
void SetupFrame::loadDefaultSetup()
{
    ...
    if (!found) 
    {
        if (mSetupList.size() == 0)
        {
            ...
            mCurrentSetup = std::shared_ptr<SimulationSetup>(new SimulationSetup());
        }
        else
            changeSetup(0);
    }
...
void SetupFrame::addSetup()
{
    if (checkSaved())
    {
        ...
        std::shared_ptr<SimulationSetup> newSetup(new SimulationSetup(dialog.getName(), dialog.getFilePath()));
        std::shared_ptr<SimulationSetup> savedSetup = mSimulationManager->saveSimulationSetup(newSetup);
...
int SetupFrame::insertSetup(std::shared_ptr<SimulationSetup> setup, bool copy)
{
    ...
    for (auto it = mSetupList.begin(); it != mSetupList.end(); it++)
    {
        ...
        {
            if (copy)
                *it = std::shared_ptr<SimulationSetup>(new SimulationSetup(*setup));
            else
                *it = setup;
    ...
    {
        if (copy)
            mSetupList.push_back(std::shared_ptr<SimulationSetup>(new SimulationSetup(*setup)));
        else
...
void SetupFrame::changeSetup(int index)
{
    ...
    mCurrentSetup = std::shared_ptr<SimulationSetup>(mSetupList.at(index));
...
void SetupFrame::loadSetup()                        
{
    if (checkSaved())
    {
        ...
        if (returning != 0)
        {
            ...
            for (int i = 0; i < files.size(); i++)
            {
                std::shared_ptr<SimulationSetup> loadedSetup = mSimulationManager->loadSimulationSetup(files.at(i), loadAlways);
...
void SetupFrame::loadSetup(QString path)
{
    ...
    //Load setup
    std::shared_ptr<SimulationSetup> loadedSetup = mSimulationManager->loadSimulationSetup(path, false);
...
void SetupFrame::updateSetupListDisplay()
{
    ...
    {        
        ...
        if (!found)
        {
            ...
            mSetupList.push_back(std::shared_ptr<SimulationSetup>(*it));
        }
    }
    ...
    for (auto it = mSetupList.begin(); it != mSetupList.end(); it++)
    {
        ...
        {
            const std::shared_ptr<SimulationSetup>& setup = mSimulationManager->getSimulation()->getSetup();
            if (setup->equals(**it))
            {
                isCurrent = true;
            }
