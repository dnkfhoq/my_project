...
public:
    ...
    */
    SceneGraphWidget(std::shared_ptr<SparkSimulationThread> thread, SceneGraphFrame* creator, QWidget* parentWidget = 0);
    /*!
    ...
    */
    ...
    /*!
     ...
     \param thread
    */
    void updateDisplay(std::shared_ptr<SparkSimulationThread> thread);
    ...
    */
    std::shared_ptr<zeitgeist::Leaf> getCurrentLeaf() const;
...
protected:
    ...
    */
    void updateDisplay(std::shared_ptr<SparkController> controller);
...
private:
    ...
    */
    void initModelView(std::shared_ptr<SparkController> controller);
    ...
    */
    bool canExpandItem(const QModelIndex& index, std::shared_ptr<zeitgeist::Leaf> leaf) const;
    /*!
     \brief Returns true if the leaf at the given index can be collapsed.
    */
    bool canCollapseItem(const QModelIndex& index, std::shared_ptr<zeitgeist::Leaf> leaf) const;
    /*!
     \brief Returns true if the leaf at the given index can be printed to the console.
    */
    bool canPrintItem(const QModelIndex& index, std::shared_ptr<zeitgeist::Leaf> leaf) const;
    /*!
     \brief Returns true if the leaf at the given index can be deleted from the scene graph.
    */
    bool canDeleteItem(const QModelIndex& index, std::shared_ptr<zeitgeist::Leaf> leaf) const;
    /*!
     \brief Returns true if the leaf at the given index can be inspected in a property widget.
    */
    bool canInspectItem(const QModelIndex& index, std::shared_ptr<zeitgeist::Leaf> leaf) const;
    /*!
     \brief Returns true if the leaf at the given index can cut.
    */
    bool canCutItem(const QModelIndex& index, std::shared_ptr<zeitgeist::Leaf> leaf) const;
    /*!
     \brief Returns true if a currently cut can be pasted at the given leaf at the given index.
    */
    bool canPasteInItem(const QModelIndex& index, std::shared_ptr<zeitgeist::Leaf> leaf) const;
    /*!
     \brief Returns true if a scene file can be loaded in the the leaf at the given index.
    */
    bool canLoadInItem(const QModelIndex& index, std::shared_ptr<zeitgeist::Leaf> leaf) const;
    /*!
     \brief Returns true if the leaf at the given index can be saved to file.
    */
    bool canSaveItem(const QModelIndex& index, std::shared_ptr<zeitgeist::Leaf> leaf) const;

    /*!
     \brief Recursively expands the children of the specified item.
    */
    void expandItem(const QModelIndex& index, std::shared_ptr<zeitgeist::Leaf> leaf);
    /*!
     \brief Recursively collapses the children of the specified item.
    */
    void collapseItem(const QModelIndex& index, std::shared_ptr<zeitgeist::Leaf> leaf);
    /*!
    ...
    */
    void printItem(const QModelIndex& index, std::shared_ptr<zeitgeist::Leaf> leaf);
    /*!
    ...
    */
    void deleteItem(const QModelIndex& index, std::shared_ptr<zeitgeist::Leaf> leaf);
    /*!
     \brief Inspects the specified item in the property widget.
    */
    void inspectItem(const QModelIndex& index, std::shared_ptr<zeitgeist::Leaf> leaf);
    /*!
     \brief Cuts the specified item from the scene graph location. Marks it for moving to a pase position.

     \todo not implemented yet
    */
    void cutItem(const QModelIndex& index, std::shared_ptr<zeitgeist::Leaf> leaf); 
    /*!
     \brief Pastes a cut item into the specified leaf.

     \todo not implemented yet
    */
    void pasteInItem(const QModelIndex& index, std::shared_ptr<zeitgeist::Leaf> leaf);
    /*!
     ...
     simulation cycle or wait cycle.
    */
    void loadInItem(const QModelIndex& index, std::shared_ptr<zeitgeist::Leaf> leaf);
    /*!
    ...
    */
    void saveItem(const QModelIndex& index, std::shared_ptr<zeitgeist::Leaf> leaf);
...
private:
    ...
    std::shared_ptr<SparkController> mSparkController; /*!< Pointer to spark controller. */
