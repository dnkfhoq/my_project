...
void SceneGraphFrame::initTaskList()
{
    ...
    {
        std::shared_ptr<SimulationTask> task(*it);
...
void SceneGraphFrame::observeSparkSimulationThread(std::shared_ptr<SparkSimulationThread> thread)
{
    if (thread->getTaskDefinition().isSpark())
    {
        connect(std::static_pointer_cast<SparkSimulationThread>(thread).get(), SIGNAL(sparkRunning(SparkController*)), this, SLOT(updateSparkRunning(SparkController*)));
        connect(std::static_pointer_cast<SparkSimulationThread>(thread).get(), SIGNAL(sparkFinished(SparkController*)), this, SLOT(updateSparkFinished(SparkController*)));
    }
}

std::shared_ptr<SparkController> SceneGraphFrame::getSparkController(std::shared_ptr<SimulationTask> task)
{
    //Stacked Widget content
    if (task->getTaskDefinition().isSpark() && task->getTaskDefinition().isThread())
    {
        //Has scene graph
        return std::static_pointer_cast<SparkSimulationThread>(task)->getSparkController();
    }
    else
    {
        //Has no scene graph
        return std::shared_ptr<SparkController>();
    }
}

std::shared_ptr<SimulationTask> SceneGraphFrame::getCurrentTask()
{
    if (mCurrentTask < 0 || mCurrentTask >= (int)mTasks.size())
        return std::shared_ptr<SimulationTask>();
...
void SceneGraphFrame::addSceneGraphWidget(int localindex)
{
    ...
    std::shared_ptr<SimulationTask> task(mTasks.at(localindex).second);
    ...
    {
        std::shared_ptr<SparkSimulationThread> thread(std::static_pointer_cast<SparkSimulationThread>(task));
...
bool SceneGraphFrame::checkAndInsertTask(std::shared_ptr<SimulationTask> task, int localIndex)
{
...
void SceneGraphFrame::updateTaskAdded(int index, int id)
{
    ...
    //Add entry to list
    std::shared_ptr<SimulationTask> task(mSimulationManager->getSimulation()->getTaskList().at(index));
...
void SceneGraphFrame::updateSparkRunning(SparkController* controller)
{
    ...
    std::shared_ptr<SimulationTask> currentTask(getCurrentTask());
    if (getSparkController(currentTask).get() == controller)
    {
        //Update current view
        mSceneGraphWidgets.at(mCurrentTask)->updateDisplay(std::static_pointer_cast<SparkSimulationThread>(currentTask));
    }
...
void SceneGraphFrame::updateCurrent()
{
    ...
    //Update current view
    std::shared_ptr<SimulationTask> updateTask(getCurrentTask());
    ...
    //Update scene graph
    mSceneGraphWidgets.at(mCurrentTask)->updateDisplay(std::static_pointer_cast<SparkSimulationThread>(updateTask));
}
