...
SparkGLRender::RenderSelectionContext::RenderSelectionContext(std::shared_ptr<BaseNode> node)
{
    assert(node.get() != 0);
    glPushMatrix();
...
bool SparkGLRender::init(std::shared_ptr<SimSpark> spark)
{
    mCamera.reset();
    mRenderServer.reset();
    ...
    //Search camera
    mCamera = std::dynamic_pointer_cast<Camera>
    ...
    //Search RenderServer
    mRenderServer = std::dynamic_pointer_cast<RenderServer>
    ...
    mRenderControl = std::dynamic_pointer_cast<RenderControl>
...
std::weak_ptr<RenderNode> SparkGLRender::pickNode(int pos_x, int pos_y, int width, int height)
{
    if ((mSpark.get() == 0) || (mRenderServer.get() == 0))
    {
        assert(false);
        return std::weak_ptr<RenderNode>();
    }
    ...
    std::weak_ptr<RenderNode> pickedNode = mRenderServer->GetPickedNode();
