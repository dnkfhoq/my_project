...
void SparkGLWidget::mouseDoubleClickEvent(QMouseEvent* e)
{
    ...
    if (e->button() == Qt::LeftButton)
    {
        ...
        if (tryLock(1000))
        {
            ...
            std::weak_ptr<kerosin::RenderNode> picked(mRender->pickNode(e->x(), e->y(), size().width(), size().height()));
            swapBuffers();
            if ((mErr = glGetError()))
                LOG_ERROR() << OpenGLManager::getErrorMessageStd(mErr).c_str() << " After picking.";

            std::shared_ptr<kerosin::RenderNode> locked(picked.lock());
            if (locked.get())
...
bool SparkGLWidget::initSpark(std::shared_ptr<SparkController> controller,
    ...
    const QString& renderServerPath, const QString& renderControlPath)
{
    ...
    //Check spark
    std::shared_ptr<SimSpark> spark = controller->getSpark();
    if (spark.get() == 0)
    {
    ...
    //Get input server
    std::shared_ptr<InputServer> inputServer = spark->GetInputServer();
    ...
    //Create renderer
    mRender = std::shared_ptr<SparkGLRender>(
        new SparkGLRender(
