...
public:
    ...
    */
    ImportScene(const QString& file, std::shared_ptr<oxygen::BaseNode> node, std::shared_ptr<zeitgeist::ParameterList> parameters) :
...
private:
    ...
    std::shared_ptr<oxygen::BaseNode> mNode; /*!< Node to load scene file in. */
    std::shared_ptr<zeitgeist::ParameterList> mParameters; /*!< Parameter list for script file. */
};
...
public:
    ...
    */
    RemoveLeaf(std::shared_ptr<zeitgeist::Leaf> leaf) :
...
private:
    ...
    std::shared_ptr<zeitgeist::Leaf> mLeaf; /*!< Leaf to unlink and remove. */
};
