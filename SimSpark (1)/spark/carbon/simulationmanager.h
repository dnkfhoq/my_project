...
public:
    ...
    */
    typedef std::vector<std::shared_ptr<SimulationSetup> > TSetupList;
...
public:
    ...
    */
    std::shared_ptr<ServerThread> getSparkServerThread() const;
    /*!
    ...
    */
    std::shared_ptr<ServerThread> initSparkServerThread(ServerThread* server);
    ...
    */
    std::shared_ptr<SimulationSetup> saveSimulationSetupByDialog(bool overwrite = false, bool saveData = false);
    /*!
    ...
    */
    std::shared_ptr<SimulationSetup> loadSimulationSetup(const QString& path, bool store = false);
    /*!
    ...
    */
    std::shared_ptr<SimulationSetup> saveSimulationSetup(std::shared_ptr<SimulationSetup> setup, bool overwrite = false, bool rememberPath = true);
    /*!
    ...
    */
    const std::shared_ptr<Simulation> getSimulation() const;
    /*!
     \brief Returns a pointer to the current Simulation or 0.
    ...
    */
    std::shared_ptr<TaskDefinition> newTaskDefinition(TaskDefinition::ETaskType type) const;
...
protected:
    ...
    */
    std::shared_ptr<SimulationSetup> loadSimulationSetup(QDomDocument& document, const QString& file, const QString& name);
    /*!
    ...
    */
    std::shared_ptr<SimulationSetup> saveCurrentSimulationSetup(int& err, const QString& path = QString());
    /*!
     ...
     \return saved or updated setup, null pointer if nothing was saved
    */
    std::shared_ptr<SimulationSetup> saveSimulationSetupProtected(std::shared_ptr<SimulationSetup> setup, int& err, bool overwrite = false, bool rememberPath = true);
    /*!
     ...
     \return saved or updated setup, null pointer if nothing was saved
    */
     std::shared_ptr<SimulationSetup> saveSimulationSetupProtected(SimulationSetup& setup, int& err, bool overwrite = false, bool rememberPath = true);
    /*!
...
private:
    ...
    std::shared_ptr<Simulation> mSimulation; /*!< Actually initialized SimulationSetup. Initially an empty Simulation is loaded.*/
    ...
    std::shared_ptr<ServerThread> mServerThread;

    std::shared_ptr<Settings> mSettings; /*!< Pointer to global settings object. */
