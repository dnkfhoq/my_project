...
SparkTreeModel::SparkTreeModel(std::shared_ptr<zeitgeist::Core> core, QObject * parent) :
...
{
    std::shared_ptr<zeitgeist::Leaf> leaf(item->leaf().lock());
    if (leaf.get() == 0)
    {
        return true;
    }
    else
    {
        ...
        bool hasItems;
        std::shared_ptr<zeitgeist::Leaf> comp;
        int row = 0;
        ...
        while (seek)
        {
            ...
            if (hasItems)
            {
                ...
                comp = std::shared_ptr<zeitgeist::Leaf>(list_at_pos->leaf().lock());
