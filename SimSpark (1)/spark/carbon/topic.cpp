...
{
    bool changed = false;
    std::shared_ptr<TopicRegistration> entry = findSender(sender);
    std::shared_ptr<TopicRegistration> newEntry = std::make_shared<TopicRegistration>(sender, signal, maxReceivers, connectionType);
    if (!entry.get())
    {
...
{
    bool changed = false;
    std::shared_ptr<TopicRegistration> entry = findReceiver(receiver);
    std::shared_ptr<TopicRegistration> newEntry = std::make_shared<TopicRegistration>(receiver, slot, maxSenders, connectionType);
    if (!entry.get())
    {
...
{
    int index = -1; 
    std::shared_ptr<TopicRegistration> entry = findSender(sender, index);
    if (entry.get() == 0)
        return false;
    if (!validPtr)
        entry->setInvalid();
...
{
    int index = -1; 
    std::shared_ptr<TopicRegistration> entry = findReceiver(receiver, index);
    if (entry.get() == 0)
        return false;
    if (!validPtr)
        entry->setInvalid();
...
std::shared_ptr<TopicRegistration> Topic::findSender(const AbstractPlugin* sender) const
{
    int i = 0;
    return findSender(sender, i);
}

std::shared_ptr<TopicRegistration> Topic::findSender(const AbstractPlugin* sender, int& index) const
{
    int i = 0;
    for (auto it = mSenders.begin(); it != mSenders.end(); it++)
    {
    ...
    return std::shared_ptr<TopicRegistration>();
}

std::shared_ptr<TopicRegistration> Topic::findReceiver(const AbstractPlugin* receiver) const
{
    int i = 0;
    return findReceiver(receiver, i);
}

std::shared_ptr<TopicRegistration> Topic::findReceiver(const AbstractPlugin* receiver, int& index) const
{
    int i = 0;
    for (auto it = mReceivers.begin(); it != mReceivers.end(); it++)
    {
...
std::shared_ptr<Topic> Topic::getSelf() const
{
