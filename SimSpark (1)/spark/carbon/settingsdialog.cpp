...
bool SettingsDialog::registerToSettingWidget(QString topic, QString page, AbstractPlugin* registree)
{
    ...
    std::shared_ptr<PageDefinition> pagePtr(getPageDefinition(topic, page));
...
void SettingsDialog::includeSettingWidget(QString topicname, QString pagename, SettingsWidget** widget, AbstractPlugin* creator, QString topicicon)
{
    ...
    std::shared_ptr<TopicDefinition> topic(createTopic(topicname, topicicon));
    std::shared_ptr<PageDefinition>  page (createPage (topicname, pagename ));
...
void SettingsDialog::removeSettingWidgets(AbstractPlugin* creator)
{
    ...
    {
        std::shared_ptr<TopicDefinition> topic(*t_it);
        //Search all pages
        for (TPageList::iterator p_it = topic->mPageList.begin(); p_it != topic->mPageList.end();)
        {
            std::shared_ptr<PageDefinition> page(*p_it);
            bool erased = false;
...
std::shared_ptr<SettingsDialog::PageDefinition> SettingsDialog::getCurrentPage()
{
    std::shared_ptr<TopicDefinition> topic(getCurrentTopic());
    if (topic.get() == 0 || topic->mTabWidget->count() == 0)
        return std::shared_ptr<PageDefinition>();

    return topic->mPageList.at(topic->mTabWidget->currentIndex());
}

std::shared_ptr<SettingsDialog::TopicDefinition> SettingsDialog::getCurrentTopic()
{
    if (mTopicList.empty())
        return std::shared_ptr<TopicDefinition>();
...
std::shared_ptr<SettingsDialog::TopicDefinition> SettingsDialog::getTopicDefinition(const QString& topicname)
{
...
std::shared_ptr<SettingsDialog::PageDefinition> SettingsDialog::getPageDefinition(const QString& topicname, const QString& pagename)
{
    std::shared_ptr<TopicDefinition> topic(getTopicDefinition(topicname));
    if (topic == 0)
        return std::shared_ptr<PageDefinition>();
...
std::shared_ptr<SettingsDialog::PageDefinition> SettingsDialog::getPageDefinition(const TopicDefinition& topic, const QString& pagename)
{
...
std::shared_ptr<SettingsDialog::TopicDefinition> SettingsDialog::createTopic(const QString& topicname, const QString& iconpath)
{
    std::shared_ptr<TopicDefinition> topic(getTopicDefinition(topicname));
    if (topic.get() == 0)
    {
        topic = std::shared_ptr<TopicDefinition>(new TopicDefinition(topicname, 0, 0));
        mTopicList.push_back(std::shared_ptr<TopicDefinition>(topic));
...
std::shared_ptr<SettingsDialog::PageDefinition> SettingsDialog::createPage(const QString& topicname, const QString& pagename)
{
    std::shared_ptr<TopicDefinition> topic(getTopicDefinition(topicname));
    if (topic.get() == 0)
        topic = std::shared_ptr<TopicDefinition>(createTopic(topicname));

    std::shared_ptr<PageDefinition> page(getPageDefinition(*topic, pagename));
    if (page.get() == 0)
    {
        page = std::shared_ptr<PageDefinition>(new PageDefinition(pagename, 0, 0, topic)); //stores topic as parent. other parameters are defined later
        topic->mPageList.push_back(std::shared_ptr<PageDefinition>(page));
        page->mWidget = 0;
        page->mTabIndex = 0;
    }
...
void SettingsDialog::updateTabWidgetPage(PageDefinition& page)
{
    ...
    std::shared_ptr<TopicDefinition> topic = page.mTopic.lock();
    if (topic.get())
    {
...
void SettingsDialog::updateSettings(int topicindex, int pageindex)
{
    ...
    std::shared_ptr<PageDefinition> page(mTopicList.at(topicindex)->mPageList.at(pageindex));
...
{
    std::shared_ptr<PageDefinition> page(getCurrentPage());
