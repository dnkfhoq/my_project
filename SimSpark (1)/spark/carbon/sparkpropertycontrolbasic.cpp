...
#define ADD_READ_PROPERTY(NAME, CAPTION, DATA, ...) \
    list.push_back(std::shared_ptr<Property>(new Property(leaf, descriptor, NAME, QObject::tr(CAPTION), DATA, false, 0, ##__VA_ARGS__)))
#define ADD_READWRITE_PROPERTY(NAME, CAPTION, DATA, VALIDATOR, ...) \
    list.push_back(std::shared_ptr<Property>(new Property(leaf, descriptor, NAME, QObject::tr(CAPTION), DATA, true, VALIDATOR, ##__VA_ARGS__)))

#define SET_VALUE(DATAPTR) setPropertyValueProtected(prop, DATAPTR)
#define GET_VALUE(TYPE) std::static_pointer_cast<const TYPE>(prop.getTempValue())->mValue
...
bool BasicControl::updateProperty(std::shared_ptr<Property> prop)
{
    if (!supports(prop->getClassNc()))
        return false;

    //Lock the leaf
    std::shared_ptr<Leaf> leaf(prop->getLeaf().lock());
    if (!leaf.get())
        return false;
...
bool BasicControl::applyPropertyChange(std::shared_ptr<Property> prop)
{
    if (!supports(prop->getClassNc()))
        return false;
...
{
    const Class& cl = *std::static_pointer_cast<Class>(leaf);

    if (prop.getName().compare("GetBundle") == 0)
    {
        // originating bundle
        std::shared_ptr<salt::SharedLibrary> bundle = cl.GetBundle();
...
{
    std::shared_ptr<BaseNode> baseNode = std::static_pointer_cast<BaseNode>(leaf);
...
{
    const Transform& trans = *std::static_pointer_cast<Transform>(leaf);
...
{
    Transform& trans = *std::static_pointer_cast<Transform>(leaf);
    //Supported setters
...
{
    const RigidBody& body = *std::static_pointer_cast<RigidBody>(leaf);
...
{
    RigidBody& body = *std::static_pointer_cast<RigidBody>(leaf);
...
bool BasicControl::genJoint(TLeafPtr leaf, const ClassDescriptor& descriptor, TProperties& list) const
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        list.push_back(std::shared_ptr<Property>(new Property(leaf, descriptor, "GetFeedbackForce("  + strIdx + ")", QObject::tr("FeedBackForce%1").arg(strIdx),  new DVector3f())));
        list.push_back(std::shared_ptr<Property>(new Property(leaf, descriptor, "GetFeedbackTorque(" + strIdx + ")", QObject::tr("FeedBackTorque%1").arg(strIdx), new DVector3f())));
    }
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        list.push_back(std::shared_ptr<Property>(new Property(leaf, descriptor, "GetBounce("               + strIdx + ")", QObject::tr("Bounce%1"              ).arg(strIdx), new DFloat())));
        list.push_back(std::shared_ptr<Property>(new Property(leaf, descriptor, "GetLowStopDeg("           + strIdx + ")", QObject::tr("LowStopDeg%1"          ).arg(strIdx), new DFloat())));
        list.push_back(std::shared_ptr<Property>(new Property(leaf, descriptor, "GetHighStopDeg("          + strIdx + ")", QObject::tr("HighStopDeg%1"         ).arg(strIdx), new DFloat())));
        list.push_back(std::shared_ptr<Property>(new Property(leaf, descriptor, "GetLowStopPos("           + strIdx + ")", QObject::tr("LowStopPos%1"          ).arg(strIdx), new DFloat())));
        list.push_back(std::shared_ptr<Property>(new Property(leaf, descriptor, "GetHighStopPos("          + strIdx + ")", QObject::tr("HighStopPos%1"         ).arg(strIdx), new DFloat())));
        list.push_back(std::shared_ptr<Property>(new Property(leaf, descriptor, "GetCFM("                  + strIdx + ")", QObject::tr("CFM%1"                 ).arg(strIdx), new DFloat())));
        list.push_back(std::shared_ptr<Property>(new Property(leaf, descriptor, "GetStopCFM("              + strIdx + ")", QObject::tr("StopCFM%1"             ).arg(strIdx), new DFloat())));
        list.push_back(std::shared_ptr<Property>(new Property(leaf, descriptor, "GetSuspensionCFM("        + strIdx + ")", QObject::tr("SuspensionCFM%1"       ).arg(strIdx), new DFloat())));
        list.push_back(std::shared_ptr<Property>(new Property(leaf, descriptor, "GetStopERP("              + strIdx + ")", QObject::tr("StopERP%1"             ).arg(strIdx), new DFloat())));
        list.push_back(std::shared_ptr<Property>(new Property(leaf, descriptor, "GetSuspensionERP("        + strIdx + ")", QObject::tr("SuspensionERP%1"       ).arg(strIdx), new DFloat())));
        list.push_back(std::shared_ptr<Property>(new Property(leaf, descriptor, "GetLinearMotorVelocity("  + strIdx + ")", QObject::tr("LinearMotorVelocity%1" ).arg(strIdx), new DFloat())));
        list.push_back(std::shared_ptr<Property>(new Property(leaf, descriptor, "GetAngularMotorVelocity(" + strIdx + ")", QObject::tr("AngularMotorVelocity%1").arg(strIdx), new DFloat())));
        list.push_back(std::shared_ptr<Property>(new Property(leaf, descriptor, "GetMaxMotorForce("        + strIdx + ")", QObject::tr("MaxMotorForce%1"       ).arg(strIdx), new DFloat())));
    }
    return true;
}
...
{
    const Joint& joint = *std::static_pointer_cast<Joint>(leaf);
...
{
    const HingeJoint& hinge = *std::static_pointer_cast<HingeJoint>(leaf);
...
{
    const UniversalJoint& univ = *std::static_pointer_cast<UniversalJoint>(leaf);
...
{
    const ContactJointHandler& cjh = *std::static_pointer_cast<ContactJointHandler>(leaf);
...
{
    ContactJointHandler& cjh = *std::static_pointer_cast<ContactJointHandler>(leaf);
...
{
    const World& world = *std::static_pointer_cast<World>(leaf);
...
{
    World& world = *std::static_pointer_cast<World>(leaf);
...
{
    std::shared_ptr<CollisionHandler> ch = std::static_pointer_cast<CollisionHandler>(leaf);
...
{
    const DragController& dc = *std::static_pointer_cast<DragController>(leaf);
...
{
    DragController& dc = *std::static_pointer_cast<DragController>(leaf);
...
{
    const MaterialSolid& mat = *std::static_pointer_cast<MaterialSolid>(leaf);
...
{
    MaterialSolid& mat = *std::static_pointer_cast<MaterialSolid>(leaf);
    //Supported setters
