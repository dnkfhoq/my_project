...
std::shared_ptr<SimSpark> PropertyList::getSpark()
{
...
void PropertyList::init(std::weak_ptr<zeitgeist::Leaf> leaf, std::shared_ptr<SparkController> controller)
{
    mLeaf.reset();

    std::shared_ptr<Leaf> locked = leaf.lock();
    if (locked.get() == 0)
        return;
    ...
    mPropertyManager = std::shared_ptr<PropertyManager>(new PropertyManager(this, controller));

    //Add PropertyControl for default classes and call init for extensions
    mPropertyManager->registerPropertyControl(std::shared_ptr<BasicControl>(new BasicControl(&*mPropertyManager)));
    mPropertyManager->init();
}

void PropertyList::refresh(std::shared_ptr<Leaf> leaf)
{
    mLeaf = leaf;
    refresh();
}

std::shared_ptr<Property> PropertyList::getProperty(int index) const
{
...
const std::shared_ptr<const PropertyManager> PropertyList::getPropertyManager() const
{
...
{
    std::shared_ptr<Leaf> locked = mLeaf.lock();
    if (locked.get() == 0)
        return;
