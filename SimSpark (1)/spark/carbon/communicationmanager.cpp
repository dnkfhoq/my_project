...
std::shared_ptr<Topic> CommunicationManager::getTopic(const QString& name)
{
    ...
    ETopicConnectionType connectionType)
{
    ...
    if (topic.get() == 0)
    {
        mTopics.push_back(std::make_shared<Topic>(this, topicName));
...
QList<std::shared_ptr<Topic>>::Iterator CommunicationManager::removeTopic(Topic* topic)
{
    for (auto it = mTopics.begin(); it != mTopics.end(); it++)
    {
        if (&**it == topic)
