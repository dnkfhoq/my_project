...
PropertyControl::PropertyControl(PropertyManager* parent, const QString& name, std::shared_ptr<PropertyExecuter> executer) 
...
void PropertyControl::createExecuter(std::shared_ptr<SparkController> controller)
{
    ...
    std::shared_ptr<GuiSimControl> simcontrol = controller->getSimulationControl().lock();
    if (simcontrol.get() == 0)
    {
    ...
    {
        mExecuter = std::shared_ptr<PropertyExecuter>(new PropertyExecuter(this));
...
void PropertyControl::queueRead(std::shared_ptr<Property>& prop)
{
    if (mExecuter.get())
    {
...
void PropertyControl::queueWrite(std::shared_ptr<Property>& prop)
{
    if (mExecuter.get())
    {
...
void PropertyControl::acceptChange(std::shared_ptr<SparkProperty::Property> prop)
{
    prop->acceptTempValue();
}

void PropertyControl::rejectChange(std::shared_ptr<SparkProperty::Property> prop)
{
    prop->rejectTempValue();
}

void PropertyControl::updated(std::shared_ptr<SparkProperty::Property> prop, bool ok)
{
