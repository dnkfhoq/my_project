...
{
    connect(this, SIGNAL(acceptChange(std::shared_ptr<SparkProperty::Property>)), parent, 
        SLOT(acceptChange(std::shared_ptr<SparkProperty::Property>)), Qt::QueuedConnection);
    connect(this, SIGNAL(rejectChange(std::shared_ptr<SparkProperty::Property>)), parent, 
        SLOT(rejectChange(std::shared_ptr<SparkProperty::Property>)), Qt::QueuedConnection);
    connect(this, SIGNAL(updated(std::shared_ptr<SparkProperty::Property>, bool)), parent, 
        SLOT(updated(std::shared_ptr<SparkProperty::Property>, bool)), Qt::QueuedConnection);
...
void PropertyExecuter::queueGet(std::shared_ptr<Property> prop)
{
...
void PropertyExecuter::queueSet(std::shared_ptr<Property> prop)
{
...
void PropertyExecuter::onCycle()
{
    ...
    if (!mSetQueue.empty())
    {
        if (mSetSemaphore.tryAcquire(1, mSetWait))
        {
            ...
            while (!mSetQueue.empty())
            {
                ...
                std::shared_ptr<Property> prop(mSetQueue.front());
    ...
    if (!mGetQueue.empty())
    {
        if (mGetSemaphore.tryAcquire(1, mGetWait))
        {
            ...
            {
                //Lock access to property
                std::shared_ptr<Property> prop(mGetQueue.front());
