...
bool PluginManager::init()
{
    ...
    if (mExtensions.get() == 0)
    {
        ...
        mExtensions = std::shared_ptr<SimulationSetup>(new SimulationSetup());
...
std::shared_ptr<PluginTableModel> PluginManager::getTableModel()
{
    return std::shared_ptr<PluginTableModel>(new PluginTableModel(mPluginMap));
}
...
bool PluginManager::loadPlugins(const SimulationSetup::TPluginDefinitions& addListRef, const SimulationSetup::TPluginDefinitions& removeList, bool reloadall, bool removeall)
{
    ...
    std::map<int, std::pair<AbstractPlugin*, std::shared_ptr<PluginDefinition>>> keeplist;
    std::map<int, std::pair<AbstractPlugin*, std::shared_ptr<PluginDefinition>>> reloadlist;
...
{
    return createPlugin(std::shared_ptr<PluginDefinition>(new PluginDefinition(definition)), addDefToSimulation);
}

AbstractPlugin* PluginManager::createPlugin(std::shared_ptr<PluginDefinition> definition, bool addDefToSimulation)
{
    ...
    if (plugin != 0)
    {
        ...
        {
            std::shared_ptr<PluginDefinition> copiedDefinition(new PluginDefinition(*definition));
...
bool PluginManager::declareExtensionPlugin(int pluginId, bool extension, bool updateCurrentSimulationSetup)
{
    ...
    if (extension == true)
    {
        ...
        if (plugin->getPluginDefinition().get() == 0)
            plugin->setPluginDefinition(std::shared_ptr<PluginDefinition>(new PluginDefinition(plugin)));
        std::shared_ptr<PluginDefinition> definition = plugin->getPluginDefinition();
    ...
    else
    {
        ...
        if (updateCurrentSimulationSetup)
        {
            Carbon::get()->getSimulationManager()->getSimulation()->getSetupNc()->insertAddPlugin(
                std::shared_ptr<PluginDefinition>(new PluginDefinition(*plugin->getPluginDefinition()))); // Add as copy
...
bool PluginManager::loadData(std::shared_ptr<Settings> data)
{
    bool loaded = false;
    for (auto it = mPluginMap.begin(); it != mPluginMap.end(); it++)
    {
...
bool PluginManager::saveData(std::shared_ptr<Settings> data)
{
    bool loaded = false;
    for (auto it = mPluginMap.begin(); it != mPluginMap.end(); it++)
    {
