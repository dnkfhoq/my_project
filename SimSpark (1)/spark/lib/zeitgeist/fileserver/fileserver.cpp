...
std::shared_ptr<salt::RFile> FileServer::OpenResource(const std::string& inName)
{
    string fname;
    if (! LocateResource(inName, fname))
        {
            GetLog()->Error() << "(FileServer::OpenResource) Cannot locate file '"
                              << inName << "'\n";
            return std::shared_ptr<salt::RFile>();
        }

    return Open(fname);
}

std::shared_ptr<salt::RFile> FileServer::Open(const string& inName)
{
    for (TLeafList::iterator i = mChildren.begin(); i != mChildren.end(); ++i)
        {
            std::shared_ptr<FileSystem> fileSys = std::static_pointer_cast<FileSystem>(*i);

            std::shared_ptr<salt::RFile> file(fileSys->Open(inName));
    ...
    std::shared_ptr<salt::RFile> file(new StdFile());
...
{
    std::shared_ptr<salt::RFile> file = Open(inName);
...
std::shared_ptr<salt::RFile> FileServer::Get(THandle handle) const
{
...
bool FileServer::Mount(const string& inFileSysName, const string& inPath)
{
    std::shared_ptr<FileSystem> fileSys =
        std::static_pointer_cast<FileSystem>(GetChild(inPath));
    ...
    fileSys = std::static_pointer_cast<FileSystem>(GetCore()->New(inFileSysName));
...
bool FileServer::Unmount(const string& inPath)
{
    ...
    std::shared_ptr<Leaf> leaf = GetChild(inPath);
...
{
  std::shared_ptr<FileSystem> fileSystem = std::static_pointer_cast<FileSystem>(GetChild(inPath));
