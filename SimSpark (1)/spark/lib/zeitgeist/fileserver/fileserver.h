...
protected:
    typedef std::map<THandle, std::shared_ptr<salt::RFile> > TFileMap;
...
public:
    ...
    */
    std::shared_ptr<salt::RFile> OpenResource(const std::string& inName);
    ...
    /** searchs each registered file system for a file with this
        ...
        returned.
    */
    std::shared_ptr<salt::RFile> Open(const std::string& inName);
     ...
     */
    std::shared_ptr<salt::RFile> Get(THandle handle) const;
