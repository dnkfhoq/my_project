...
bool Core::CacheKey::operator < (const CacheKey& key) const
{
    ...
    std::shared_ptr<Leaf> myRoot = root.lock();
    std::shared_ptr<Leaf> keyRoot = key.root.lock();
...
Core::Core()
{
    mClassClass = std::shared_ptr<Class>(new CLASS(Class));
    mClassClass->Construct(mClassClass, std::shared_ptr<Class>());
}
...
void Core::Construct(const std::weak_ptr<Core>& self)
{
    mSelf = self;
    ...
    mRoot = std::static_pointer_cast<Leaf>(mNodeClass->Create());
    mRoot->SetName("");

    std::shared_ptr<CoreContext> context = CreateContext();
    ...
    mLogServer = std::static_pointer_cast<LogServer>
...
void Core::CatchSignal(int sig_num)
{
    ...
    // TODO: use the stacktrace functionality from C++23 to make this available
    // independently of the presence of glibc.
...
std::shared_ptr<CoreContext> Core::CreateContext()
{
    return std::shared_ptr<CoreContext>(new CoreContext(mSelf.lock(), GetRoot()));
}
...
Core::New(const std::string& className)
{
    ...
    std::shared_ptr<CoreContext> context = CreateContext();
    std::shared_ptr<Class> theClass = std::dynamic_pointer_cast<Class>
...
bool Core::RegisterClassObject(const std::shared_ptr<Class> &classObject,
...
{
    return RegisterClassObject(std::shared_ptr<Class>(classObject), subDir);
}
...
bool Core::ImportBundle(const std::string& bundleName)
{
    ...
    std::shared_ptr<SharedLibrary> bundle(new SharedLibrary());
    ...
    void(*Zeitgeist_RegisterBundle)(std::list <std::shared_ptr<Class> > &) = NULL;

    Zeitgeist_RegisterBundle = (void(*)(std::list <std::shared_ptr<Class> > &))
...
const std::shared_ptr<FileServer>& Core::GetFileServer() const
{
    return mFileServer;
}

const std::shared_ptr<LogServer>& Core::GetLogServer() const
{
    return mLogServer;
}

const std::shared_ptr<ScriptServer>& Core::GetScriptServer() const
{
    return mScriptServer;
}

std::weak_ptr<Leaf> Core::GetCachedInternal(const CacheKey& key)
{
    // lookup the key in the internal cache
    TPathCache::iterator iter = mPathCache.find(key);
    if (iter != mPathCache.end())
        {
            std::weak_ptr<Leaf>& entry = (*iter).second;
            if (! entry.expired())
                {
                    return entry;
                }
    ...
    return std::shared_ptr<Leaf>();
}

void Core::PutCachedInternal(const CacheKey& key, const std::weak_ptr<Leaf>& leaf)
{
...
std::shared_ptr<Leaf> Core::GetUncachedInternal(const CacheKey& key)
{
    // walk the hierarchy
    std::shared_ptr<Leaf> current;
    Path path(key.path);
...
std::shared_ptr<Leaf> Core::GetInternal(const std::string &pathStr,
                                          const std::shared_ptr<Leaf>& leaf)
{
    // lookup the path in the internal cache
    CacheKey key(leaf, pathStr);

    std::weak_ptr<Leaf> cached(GetCachedInternal(key));
    if (! cached.expired())
        {
            return cached.lock();
        }
...
std::shared_ptr<Leaf> Core::Get(const std::string &pathStr)
{
...
std::shared_ptr<Leaf> Core::Get(const std::string &pathStr,
                                  const std::shared_ptr<Leaf>& leaf)
{
    std::shared_ptr<Leaf> current = GetInternal(pathStr,leaf);
...
std::shared_ptr<Leaf> Core::GetChild(const std::shared_ptr<Leaf> &parent,
                                       const std::string &childName)
{
    // if we have a leaf, then we can't get a child ... ever!!
    if (parent->IsLeaf()) return std::shared_ptr<Leaf>();

    // ok, no leaf, so let's try to get an existing child
    std::shared_ptr<Leaf> child = parent->GetChild(childName);

    if (child.get() == NULL)
        {
            // if we hit this branch, then no child with the desired
            // name exists, but we will change this :)
            child = std::static_pointer_cast<Leaf>(mNodeClass->Create());
...
void Core::BindClass(const std::shared_ptr<Class> &newClass) const
{
