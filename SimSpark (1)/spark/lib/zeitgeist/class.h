...
#include <zeitgeist/scriptserver/private/gcvalue.h>
...
#define ZEITGEIST_EXPORT_BEGIN()\
    ...
    extern "C"{\
    SHARED_LIB_EXPORT void Zeitgeist_RegisterBundle(std::list <std::shared_ptr<Class> > &classes){

#define ZEITGEIST_EXPORT_EX(className, path)\
    classes.push_back(std::shared_ptr<Class>(new CLASS(className)));
...
private:
    ...
    typedef std::list< std::weak_ptr<Object> > TObjectList;
...
public:
    ...
    void SetBundle(const std::shared_ptr<salt::SharedLibrary> &bundle);
    ...
    std::shared_ptr<salt::SharedLibrary>  GetBundle() const;

protected:
    /** adds an instance to the local list of instances */
    void AttachInstance(const std::weak_ptr<Object> &instance);

    /** removes an instance from the local list of instances */
    void DetachInstance(const std::weak_ptr<Object> &instance);
...
private:
    ...
    void AttachTo(const std::weak_ptr<Core>& core);
...
private:
     ...
     */
    std::shared_ptr<salt::SharedLibrary>  mBundle;
