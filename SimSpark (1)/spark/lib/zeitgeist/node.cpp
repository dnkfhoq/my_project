...
{
    std::shared_ptr<Leaf> leaf   = Leaf::GetChild(name, recursive);
    ...
    for (TLeafList::iterator i = mChildren.begin(); i != mChildren.end(); ++i)
    {
        ...
        if (recursive)
        {
            std::shared_ptr<Leaf> ret = (*i)->GetChild(name, recursive);
...
Node::GetChildOfClass(const std::string& name, bool recursive)
{
    for (TLeafList::iterator i = mChildren.begin(); i != mChildren.end(); ++i)
    {
        ...
        std::shared_ptr<Class> theClass = (*i)->GetClass();
        ...
        if (recursive)
        {
            std::shared_ptr<Leaf> ret = (*i)->GetChildOfClass(name, recursive);
            if (ret.get() != NULL) return ret;
        }
    }
...
Node::GetChildSupportingClass(const std::string& name, bool recursive)
{
    for (TLeafList::iterator i = mChildren.begin(); i != mChildren.end(); ++i)
    {
        ...
        if (recursive)
        {
            std::shared_ptr<Leaf> ret = (*i)->GetChildSupportingClass(name, recursive);
            if (ret.get() != NULL) return ret;
        }
    }
...
void
Node::RemoveChildReference(const std::shared_ptr<Leaf>& leaf)
{
...
Node::UnlinkChildren()
{
    ...
    {
        std::shared_ptr<Leaf> node = mChildren.front();
        node->UnlinkChildren();
        node->Unlink();
    }
}
...
bool
Node::AddChildReference(const std::shared_ptr<Leaf>& leaf)
{
    if (leaf.get() == 0)
    {
        return false;
    ...
    leaf->SetParent(std::static_pointer_cast<Node>(GetSelf().lock()));
    return true;
}
