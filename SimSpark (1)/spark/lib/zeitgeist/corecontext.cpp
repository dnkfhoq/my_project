...
CoreContext::CoreContext(const std::shared_ptr<Core> &core, const std::shared_ptr<Leaf> &root) : mCore(core), mObject(root)
{
}
...
std::shared_ptr<Leaf> CoreContext::New(const std::string& className, const std::string& pathStr)
{
    // first, we try to create the class with the core
    std::shared_ptr<Object> instance = mCore->New(className);

    if (instance.get())
        {
            // now, we check the type by dynamic casting to leaf
            std::shared_ptr<Leaf> leaf = std::static_pointer_cast<Leaf>(instance);
...
{
    std::shared_ptr<Leaf> leaf = Get(name);
...
std::shared_ptr<Leaf> CoreContext::Select(const std::string &pathStr)
{
    std::shared_ptr<Leaf> leaf = Get(pathStr);
...
bool CoreContext::Install(const std::shared_ptr<Leaf> &leaf, const std::string &pathStr, bool isNamed)
{
...
std::shared_ptr<Leaf> CoreContext::Get(const std::string& pathStr)
{
