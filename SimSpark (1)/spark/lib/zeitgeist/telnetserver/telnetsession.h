...
public:
    ...
    TelnetSession(std::shared_ptr<TelnetDaemon> daemon);
    virtual ~TelnetSession();

    /** receives and remebers the socket, the local adress and a reference to the ScriptServer */
    void Init(std::shared_ptr<rcss::net::Socket> clientSocket, rcss::net::Addr clientAddr, std::shared_ptr<ScriptServer> scriptServer);
