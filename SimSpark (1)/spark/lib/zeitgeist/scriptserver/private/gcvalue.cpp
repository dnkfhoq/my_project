...
*/
#include "rubyapi.h" // might define STR2CSTR
#include "gcvalue.h"
#include <sstream>
...
ScriptValue GCValue::AsScriptValue()
{
    if (IsNil())
    {
        return ScriptValue();
    ...
    case T_STRING:
        {
            std::string str = STR2CSTR(mValue);
            return ScriptValue(str);
        }
    case T_FLOAT:
        {
            return ScriptValue((float) NUM2DBL(mValue));
        }
    case T_FIXNUM:
        {
            return ScriptValue(NUM2INT(mValue));
        }
    case T_TRUE:
        {
