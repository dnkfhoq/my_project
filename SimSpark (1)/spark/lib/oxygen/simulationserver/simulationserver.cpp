...
{
    std::shared_ptr<Leaf> prevCtrNode = GetChild(name);
    ...
    std::shared_ptr<SimControlNode> control
        = std::dynamic_pointer_cast<SimControlNode>(GetCore()->New(className));
...
bool SimulationServer::InitTimerSystem(const std::string& className)
{
    ...
    mTimerSystem = std::dynamic_pointer_cast<TimerSystem>(GetCore()->New(className));
...
std::shared_ptr<SimControlNode>
SimulationServer::GetControlNode(const string& controlName)
{
    std::shared_ptr<SimControlNode> ctrNode =
        std::dynamic_pointer_cast<SimControlNode>(GetChild(controlName));
...
void SimulationServer::ControlEvent(EControlEvent event)
{
    for (
        ...
        {
            std::shared_ptr<SimControlNode> ctrNode =
                std::dynamic_pointer_cast<SimControlNode>(*iter);
...
std::shared_ptr<GameControlServer> SimulationServer::GetGameControlServer()
{
    return mGameControlServer.get();
}

std::shared_ptr<MonitorServer> SimulationServer::GetMonitorServer()
{
    return mMonitorServer.get();
}

std::shared_ptr<SceneServer> SimulationServer::GetSceneServer()
{
...
void SimulationServer::RunMultiThreaded()
{
    ...
    mThreadBarrier = new boost::barrier(count);

    // create new threads for each SimControlNode
    std::vector<std::thread> ctrThrdGroup;
    for ( TLeafList::iterator iter=begin(); iter != end(); ++iter )
        {
            std::shared_ptr<SimControlNode> ctrNode =  std::dynamic_pointer_cast<SimControlNode>(*iter);
            if (ctrNode.get() == 0) continue;

            ctrThrdGroup.emplace_back(&SimulationServer::SimControlThread, this, ctrNode);
        }

    std::shared_ptr<SimControlNode> renderControl = GetControlNode("RenderControl");
    ...
    // wait for threads
    for (std::thread& thrd : ctrThrdGroup)
    {
        thrd.join();
    }
}

void SimulationServer::SimControlThread(std::shared_ptr<SimControlNode> controlNode)
{
    if (!mThreadBarrier)
        {
            GetLog()->Error()
