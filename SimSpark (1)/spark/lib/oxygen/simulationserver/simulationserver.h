...
public:
    ...
    std::shared_ptr<oxygen::SimControlNode>
    ...
    */
    ...
    std::shared_ptr<MonitorServer> GetMonitorServer();

    /** returns the cached GameControlServer reference */
    std::shared_ptr<GameControlServer> GetGameControlServer();

    /** returns the cached SceneServer reference */
    std::shared_ptr<SceneServer> GetSceneServer();
...
protected:
     ...
     *  multi-threaded mode. */
    void SimControlThread(std::shared_ptr<SimControlNode> controlNode);
...
protected:
    ...
    std::shared_ptr<TimerSystem> mTimerSystem;
};
