...
bool NetMessage::Extract(std::shared_ptr<NetBuffer> buffer, std::string& msg)
{
    if (buffer.get() == 0)
    {
        return false;
    ...
    const unsigned int preSz = sizeof(std::uint32_t);
    ...
    unsigned int msgLen =
            ntohl((*reinterpret_cast<const std::uint32_t*>(data.data())));
