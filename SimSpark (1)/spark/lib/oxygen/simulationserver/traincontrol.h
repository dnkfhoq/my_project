...
public:
    ...
    public:
        ...
        std::shared_ptr<oxygen::Behavior> behavior;

        /** command message received from the integrated agent */
        std::string command;

        /** predicatelist sent to the integrated agent */
        std::shared_ptr<oxygen::PredicateList> senselist;

    public:
        Client() : id(-1) {};
        Client(int i,
               std::shared_ptr<oxygen::Behavior> b =
               std::shared_ptr<oxygen::Behavior>()
               ) : id(i), behavior(b) {};
    };

    typedef std::set<std::shared_ptr<Client> > TClientSet;
...
public:
    ...
    */
    ...
    void ClientConnect(std::shared_ptr<Behavior> behavior);

    /** called when an integrated agent unregister */
    void ClientDisconnect(std::shared_ptr<Behavior> behavior);
