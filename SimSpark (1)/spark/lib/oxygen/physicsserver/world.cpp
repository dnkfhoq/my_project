...
std::shared_ptr<WorldInt> World::mWorldImp;
...
{
    if (mWorldImp.get() == 0)
        mWorldImp = std::dynamic_pointer_cast<WorldInt>
...
void World::DestroyWorldObjects()
{
  ...
  std::shared_ptr<World> self = std::static_pointer_cast<World>(GetSelf().lock());

  for (TLeafList::iterator iter = objects.begin(); iter != objects.end(); ++iter)
  {
    std::shared_ptr<PhysicsObject> object = std::static_pointer_cast<PhysicsObject>(*iter);
    if (object == self)
    {
      continue;
    }
