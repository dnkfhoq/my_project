...
public:
    ...
    void UpdateCache(std::shared_ptr<Scene> acticeScene);
    ...
    void SetUpBox(std::shared_ptr<RigidBody> body, std::string name);
    
private:
    static std::shared_ptr<PhysicsServerInt> mPhysicsServerImp;
    
    /** cached reference to the Space node below the active scene */
    std::shared_ptr<Space> mActiveSpace;

    /** cached reference to the World node below the active scene */
    std::shared_ptr<World> mActiveWorld;
};
