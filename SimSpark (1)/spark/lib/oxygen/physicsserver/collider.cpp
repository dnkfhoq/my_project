...
std::shared_ptr<ColliderInt> Collider::mColliderImp;
...
void Collider::OnLink()
{
    ...
    if (mColliderImp.get() == 0)
        mColliderImp = std::dynamic_pointer_cast<ColliderInt>
            (GetCore()->New("ColliderImp"));
    
    if (mGeomID == 0) return;
    
    std::weak_ptr<Node> parent = GetParent();
    if (parent.expired()) return; 

    std::shared_ptr<TransformCollider> tcParent =
        std::dynamic_pointer_cast<TransformCollider>(parent.lock());
    ...
    std::shared_ptr<RigidBody> body = std::static_pointer_cast<RigidBody>
...
bool Collider::AddCollisionHandler(const std::string& handlerName)
{
    std::shared_ptr<CollisionHandler> handler =
        std::dynamic_pointer_cast<CollisionHandler>(GetCore()->New(handlerName));
...
void Collider::OnCollision (std::shared_ptr<Collider> collidee,
...
{    
    ...
    for (
        ...
        {
            std::shared_ptr<CollisionHandler> handler =
                std::static_pointer_cast<CollisionHandler>(*iter);
...
std::shared_ptr<Collider> Collider::GetCollider(long geomID)
{
    if (geomID == 0)
        {
    ...
    if (collider.get() == 0)
        {
            // we cannot use the logserver here
            cerr << "ERROR: (Collider) got no std::shared_ptr for GeomID "
                 << geomID << "\n";
        }
...
bool Collider::Intersects(std::shared_ptr<Collider> collider)
{
    if (
        (mGeomID == 0) ||
        (collider.get() == 0)
...
bool Collider::InNotCollideWithSet( std::shared_ptr<Collider>  col2 )
{
