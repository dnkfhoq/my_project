...
std::shared_ptr<PhysicsObjectInt> PhysicsObject::mPhysicsObjectImp;
...
void PhysicsObject::OnLink()
{
    if (mPhysicsObjectImp.get() == 0)
        mPhysicsObjectImp = std::dynamic_pointer_cast<PhysicsObjectInt>
...
std::shared_ptr<World> PhysicsObject::GetWorld() const
{
    ...
    std::shared_ptr<World> worldNode = std::dynamic_pointer_cast<World>
...
std::shared_ptr<Space> PhysicsObject::GetSpace()
{
    // try to find the nearest parent space object
    std::weak_ptr<Space> parentSpace = FindParentSupportingClass<Space>();
    ...
    std::shared_ptr<Space> spaceNode = std::dynamic_pointer_cast<Space>
...
{
    std::shared_ptr<Space> space = GetSpace();
    if (space.get() == 0)
        {
            return 0;
        }
