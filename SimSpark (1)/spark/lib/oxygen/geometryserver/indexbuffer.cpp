...
void IndexBuffer::EnsureFit(unsigned int count)
{
    if(mIndex.get() == 0)
        {
            mIndex    = std::shared_ptr<unsigned int[]>(new unsigned int[count]);
            mMaxIndex = count;
            mNumIndex = 0;
        } else
            {
                ...
                if(mMaxIndex < reqSize)
                {
                    ...
                    // allocate new memory
                    std::shared_ptr<unsigned int[]> newIndex(new unsigned int[newSize]);
...
std::shared_ptr<unsigned int[]> IndexBuffer::GetIndex() const
{
    return mIndex;
}
