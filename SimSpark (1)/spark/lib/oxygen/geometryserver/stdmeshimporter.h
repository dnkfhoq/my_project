...
public:
    ...
    /** generates a mesh referenced by the given name and optional
        list of parameters */
    virtual std::shared_ptr<TriMesh> ImportMesh
...
protected:
    ...
    std::shared_ptr<TriMesh> UnitSphereMesh();

    /** constructs a unit box */
    std::shared_ptr<TriMesh> UnitBoxMesh();

    /** constructs a unit capsule */
    std::shared_ptr<TriMesh> UnitCapsule
    (const zeitgeist::ParameterList& parameter);

    /** constructs a flat unit cylinder */
    std::shared_ptr<TriMesh> UnitCylinder();
};
