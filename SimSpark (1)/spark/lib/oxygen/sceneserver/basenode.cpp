...
void BaseNode::SetWorldTransform(const salt::Matrix& transform)
{
    ...
    std::static_pointer_cast<BaseNode>
...
void BaseNode::UpdateCache(bool recursive)
{
    ...
    if (recursive)
    {
        ...
        for (TLeafList::iterator i = mBaseNodeChildren.begin(); i!= mBaseNodeChildren.end(); ++i)
            {
                std::static_pointer_cast<BaseNode>(*i)->UpdateCache();
            }
    }
}
...
void BaseNode::PrePhysicsUpdate(float deltaTime)
{
    ...
    for (TLeafList::iterator i = children.begin(); i!= children.end(); ++i)
        {
            std::static_pointer_cast<BaseNode>(*i)->PrePhysicsUpdate(deltaTime);
        }
...
void BaseNode::PostPhysicsUpdate()
{
    ...
    for (TLeafList::iterator i = mBaseNodeChildren.begin(); i!= mBaseNodeChildren.end(); ++i)
        {
            std::static_pointer_cast<BaseNode>(*i)->PostPhysicsUpdate();
        }
...
std::shared_ptr<Scene> BaseNode::GetScene() const
{
    // is this node the scene node ?
    std::shared_ptr<Scene> self =
        std::dynamic_pointer_cast<Scene>(GetSelf().lock());
...
bool BaseNode::ImportScene(const string& fileName, std::shared_ptr<ParameterList> parameter)
{
    std::shared_ptr<SceneServer> sceneServer = std::dynamic_pointer_cast<SceneServer>
    ...
    std::shared_ptr<BaseNode> node =
        std::dynamic_pointer_cast<BaseNode>(GetSelf().lock());
