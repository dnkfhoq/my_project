...
AgentAspect::AgentAspect() : Transform(), mSyncEnabled(false), mIsSynced(true)
{
    SetName("agentAspect");
    mID = -1;
    mPerceptorCycle = 0;
...
bool
AgentAspect::RealizeActions(std::shared_ptr<ActionObject::TList> actions)
{
    for (
        ...
        {
            std::shared_ptr<ActionObject> action = (*iter);
            std::string predicate = action->GetPredicate();

            std::shared_ptr<Effector> effector = GetEffector(predicate);
            if (effector.get() == 0)
                {
                    GetLog()->Warning()
...
std::shared_ptr<PredicateList>
AgentAspect::QueryPerceptors()
{
    mPerceptorCycle++;
    // build list of perceptors, searching recursively

    std::shared_ptr<PredicateList> predList(new PredicateList());
    ...
    for (
        ...
        {
            std::shared_ptr<Perceptor> pct = std::static_pointer_cast<Perceptor>(*iter);
...
AgentAspect::UpdateEffectorMap()
{
    ...
    for (
        ...
        {
            std::shared_ptr<Effector> effector = std::static_pointer_cast<Effector>(*iter);
...
AgentAspect::Init(const string& createEffector, int id)
{
    ...
    std::shared_ptr<Effector> create = std::dynamic_pointer_cast<Effector>
...
void AgentAspect::EnableSync(bool sync)
{
    mSyncEnabled = sync;
}
...
{
    // If synchronization is not enabled for this agent, the agent is viewed as
    // being synchronized to avoid freezing the simulation.
    return mIsSynced || mPerceptorCycle < 5 || !mSyncEnabled;
}
