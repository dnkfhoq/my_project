...
{
    GetLog()->Normal() << "(InputServer) Create " << inputSysName << "\n";
    Reset();
    ...
    std::shared_ptr<InputSystem> inputSystem =
        std::dynamic_pointer_cast<InputSystem>(GetCore()->New(inputSysName));
...
bool InputServer::InitInputSystem()
{
    GetLog()->Normal() << "(InputServer) Init InputSystem\n";

    std::shared_ptr<InputSystem> inputSystem = GetInputSystem();
    if (inputSystem.get() == 0)
    {
        GetLog()->Error()
...
std::shared_ptr<InputSystem> InputServer::GetInputSystem()
{
    std::shared_ptr<InputSystem> inputSystem = std::dynamic_pointer_cast<InputSystem>
