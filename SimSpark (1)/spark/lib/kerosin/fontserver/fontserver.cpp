...
std::shared_ptr<kerosin::Font> FontServer::GetFont(const string &name, unsigned int size)
{
    std::shared_ptr<kerosin::Font> theFont = FindFont(name, size);
    ...
    if(! LoadFont(name, size, theFont))
        {
            return std::shared_ptr<kerosin::Font>();
        }
...
std::shared_ptr<kerosin::Font> FontServer::FindFont(const string &name,
...
bool FontServer::LoadFont(const string &name, unsigned int size,
                          std::shared_ptr<kerosin::Font> &font)
{
    std::shared_ptr<salt::RFile> file = GetFile()->OpenResource(name);
