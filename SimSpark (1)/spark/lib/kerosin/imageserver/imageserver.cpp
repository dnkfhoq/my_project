...
std::shared_ptr<FileServer> gFileServer;
...
ImageServer::Load(const string& inName, ImageServer::EImgType inType) const
{
    ...
    std::shared_ptr<FileServer> fileServer
        = std::static_pointer_cast<FileServer>(GetCore()->Get("/sys/server/file"));
    ...
    std::shared_ptr<RFile> rfile = fileServer->OpenResource(inName);

    std::unique_ptr<unsigned char[]> buffer(new unsigned char[rfile->Size()]);
