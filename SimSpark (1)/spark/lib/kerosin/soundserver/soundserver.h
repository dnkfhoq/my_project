...
private:
#ifdef HAVE_HASH_MAP
    typedef std::hash_map<std::string, std::shared_ptr<SoundObject> >     TSoundHashMap;
#else
    typedef std::map<std::string, std::shared_ptr<SoundObject> >  TSoundHashMap;
#endif
...
public:
    ...
    std::shared_ptr<SoundEffect>  LoadEffect(const std::string& inName);
    std::shared_ptr<SoundStream>  LoadStream(const std::string& inName);
    std::shared_ptr<SoundModule>  LoadModule(const std::string& inName);
...
private:
    ...
    bool    LoadSoundObject(const std::string& inName, const TSoundHashMap& map, std::shared_ptr<SoundObject> &soundObject) const;
