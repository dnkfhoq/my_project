...
bool
DriveEffector::Realize(std::shared_ptr<ActionObject> action)
{
    if (mBody.get() == 0)
    {
        return false;
    ...
    std::shared_ptr<BaseNode> parent =
        std::dynamic_pointer_cast<BaseNode>(GetParent().lock());
    ...
    std::shared_ptr<DriveAction> driveAction = std::dynamic_pointer_cast<DriveAction>(action);
...
DriveEffector::GetActionObject(const Predicate& predicate)
{
  ...
  if (! predicate.GetValue(predicate.begin(), force))
  {
      ...
      return std::shared_ptr<ActionObject>(new ActionObject(GetPredicate()));
  }

  return std::shared_ptr<ActionObject>(new DriveAction(GetPredicate(),force));
}
...
DriveEffector::OnLink()
{
    ...
    std::shared_ptr<SphereCollider> geom =
        std::dynamic_pointer_cast<SphereCollider>(mTransformParent->GetChild("geometry"));
