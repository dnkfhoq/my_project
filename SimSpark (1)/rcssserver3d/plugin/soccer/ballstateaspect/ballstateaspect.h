...
public:
    ...
    */
    ...
    /** updates the reference to the last agent that collided with the
        ball
    */
    void UpdateLastCollidingAgent(std::shared_ptr<oxygen::AgentAspect> agent);

    /** updates the reference to the last agent that kicked the ball */
    void UpdateLastKickingAgent(std::shared_ptr<oxygen::AgentAspect> agent);
...
protected:
    ...
    std::shared_ptr<oxygen::RecorderHandler> mBallRecorder;

    /** reference to the left goal recorder */
    std::shared_ptr<oxygen::RecorderHandler> mLeftGoalRecorder;

    /** reference to the right goal recorder */
    std::shared_ptr<oxygen::RecorderHandler> mRightGoalRecorder;

    /** list of references to agents currently colliding with the ball */
    std::list<std::shared_ptr<oxygen::AgentAspect> > mCollidingAgents;

    /** holds a reference to the last agent that collided with the
        ball */
    std::shared_ptr<oxygen::AgentAspect> mLastCollidingAgent;

    /** holds a reference to the last agent that kicked the ball */
    std::shared_ptr<oxygen::AgentAspect> mLastKickingAgent;
