...
bool BallStateAspect::GetCollidingAgents(list<std::shared_ptr<AgentAspect> >& agents)
{
    agents = mCollidingAgents;
    return agents.size() > 0;
}

bool BallStateAspect::GetLastCollidingAgent(std::shared_ptr<AgentAspect>& agent,
                                            TTime& time)
{
...
bool BallStateAspect::GetLastKickingAgent(std::shared_ptr<AgentAspect>& agent,
                                            TTime& time)
{
...
void BallStateAspect::UpdateCollidingAgents()
{
    mCollidingWithLeftTeamAgent = false;
    mCollidingWithRightTeamAgent = false;
    mCollidingAgents = list<std::shared_ptr<AgentAspect> >();
    ...
    if (agents.size() > 0)
        {
            mLastCollidingAgent = std::static_pointer_cast<AgentAspect>
                (agents.front().lock());
            ...
            for (RecorderHandler::TParentList::iterator it = agents.begin(); 
                ...
                std::shared_ptr<oxygen::AgentAspect> agent = 
                    std::static_pointer_cast<AgentAspect>(it->lock());   
...
void
BallStateAspect::UpdateLastCollidingAgent(std::shared_ptr<AgentAspect> agent)
{
...
void
BallStateAspect::UpdateLastKickingAgent(std::shared_ptr<AgentAspect> agent)
{
