...
KickEffector::PrePhysicsUpdateInternal(float /*deltaTime*/)
{
    ...
    std::shared_ptr<KickAction> kickAction =
        std::dynamic_pointer_cast<KickAction>(mAction);
    mAction.reset();
...
KickEffector::GetActionObject(const Predicate& predicate)
{
  do
  {
      ...
      return std::shared_ptr<KickAction>(new KickAction(GetPredicate(),angle,power));
...
KickEffector::OnLink()
{
    ...
    std::shared_ptr<SphereCollider> geom =
        std::dynamic_pointer_cast<SphereCollider>(mAgent->GetChild("geometry"));
    if (geom.get() == 0)
    {
        GetLog()->Error()
