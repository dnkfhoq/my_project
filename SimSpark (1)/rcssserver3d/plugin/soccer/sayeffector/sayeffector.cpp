...
bool
SayEffector::Realize(std::shared_ptr<ActionObject> action)
{
    ...
    std::shared_ptr<SayAction> sayAction =
        std::dynamic_pointer_cast<SayAction>(action);
    ...
    // If characters outside allowed value (0x20 to 0x7E; inclusive) range or " " (0x20), "(" or ")" are in mMessage, return false
    if (!std::all_of(mMessage.begin(), mMessage.end(), [](char c){ return c >= 0x21 && c <= 0x7E && c != '(' && c != ')'; }))
    {
        std::string teamName = mAgentState->GetTeamIndex() == TI_RIGHT ? "Right" : "Left";

        // get the GameStateAspect
        std::shared_ptr<GameStateAspect> mGameState = std::dynamic_pointer_cast<GameStateAspect>
...
SayEffector::GetActionObject(const Predicate& predicate)
{
    ...
    return std::shared_ptr<SayAction>(new SayAction(GetPredicate(), message));
...
SayEffector::OnLink()
{
    ...
    mAgent = std::dynamic_pointer_cast<AgentAspect>(GetParent().lock());
