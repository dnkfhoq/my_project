    ...
    mSettings(getCarbon()->getSettings())
{
    ...
    connect(simulation, SIGNAL(serverCreated(std::shared_ptr<ServerThread>)), this, SLOT(updateServerCreated(std::shared_ptr<ServerThread>)));
...
bool SoccerControlFrame::tryInitSpark()
{
    ...
    if (mSparkController->getSpark().get() != 0)
    {
        ...
        std::shared_ptr<GameControlServer> gameControl(std::dynamic_pointer_cast<GameControlServer>(spark->GetCore()->Get(gameControlServer.toStdString())));
        std::shared_ptr<GameStateAspect>   gameState  (std::dynamic_pointer_cast<GameStateAspect>  (spark->GetCore()->Get(gameStateAspect.toStdString())));
        std::shared_ptr<SoccerRuleAspect>  soccerRule (std::dynamic_pointer_cast<SoccerRuleAspect> (spark->GetCore()->Get(soccerRuleAspect.toStdString())));
        std::shared_ptr<BallStateAspect>   ballState  (std::dynamic_pointer_cast<BallStateAspect>  (spark->GetCore()->Get(ballStateAspect.toStdString())));
...
{
    std::shared_ptr<GuiSimControl> simControl = mSparkController->getSimulationControl().lock();
    if (simControl.get() == 0)
    {
    ...
    //Create new updater
    mUpdater = std::shared_ptr<SoccerFrameUpdateControl>(new SoccerFrameUpdateControl(this));
...
void SoccerControlFrame::updateServerCreated(std::shared_ptr<ServerThread> server)
{
...
void SoccerControlFrame::editGameTime()
{
    ...
    mUpdater->queueCommand(std::make_shared<SoccerControlFrameUtil::SetTime>(mGameAspect, ui.gameTimeEdit->text().toFloat()));
}
...
void SoccerControlFrame::resetGameTime()
{
    ...
    mUpdater->queueCommand(std::make_shared<SoccerControlFrameUtil::SetTime>(mGameAspect, 0.0));
}
...
void SoccerControlFrame::editTeam1Goals()
{
    ...
    mUpdater->queueCommand(std::make_shared<SoccerControlFrameUtil::SetTeamGoals>(mGameAspect, TI_LEFT, ui.team1GoalsEdit->text().toInt()));
}
...
void SoccerControlFrame::editTeam2Goals()
{
    ...
    mUpdater->queueCommand(std::make_shared<SoccerControlFrameUtil::SetTeamGoals>(mGameAspect, TI_LEFT, ui.team2GoalsEdit->text().toInt()));
}
...
void SoccerControlFrame::changePlayMode()
{
    ...
    mUpdater->queueCommand(std::make_shared<SoccerControlFrameUtil::ChangePlayMode>(mGameAspect, mSelectedPlayMode));
}
...
void SoccerControlFrame::kickOff()
{
    ...
    mUpdater->queueCommand(std::make_shared<SoccerControlFrameUtil::KickOff>(mGameAspect));
}
