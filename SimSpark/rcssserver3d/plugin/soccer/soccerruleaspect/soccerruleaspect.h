...
{
public:
    typedef std::list<std::shared_ptr<AgentState> > TAgentStateList;
    ...
    struct Foul
    {
        Foul(unsigned _index, EFoulType _type, std::shared_ptr<AgentState> _agent)
            : index(_index),
              type(_type),
              agent(_agent)
        {}
...
public:
    ...
    */
    ...
    /** Checks if there are any ball holding fouls
     */
    void AnalyseBallHoldingFouls();
     ...
     */
    void GetTreeBoxColliders(std::shared_ptr<zeitgeist::Leaf> root, 
                             std::vector< std::shared_ptr<oxygen::BoxCollider> > &agentBoxes
                   ); 
    ...
    */
     ...
     */
    void PenalizeTouchingFoul(std::shared_ptr<AgentState> agentState, const salt::Vector3f touchGroupCenter);
    ...
    */
    void ClearPlayersWithException(const salt::Vector3f& pos, float radius, float min_dist,
                      TTeamIndex idx, std::shared_ptr<AgentState> agentState);
    ...
    /** Get the left penalty area */
    salt::AABB2 GetLeftPenaltyArea() const;

    /** Get the right penalty area */
    salt::AABB2 GetRightPenaltyArea() const;
    ...
    */
    ...
    /** indicates that a kick pass is currently in progress */
    void StartKickPass(TTeamIndex idx);

    /** indicates that a kick pass ended */
    void EndKickPass();

    /** true, if a kick pass is currently in progress */
    bool IsKickPassActive();

    void SetPlayerUNumTouchedBallSincePassMode(TTeamIndex idx, int unum);

    void SetMulitpleTeammatesTouchedBallSincePassMode(TTeamIndex idx, bool touched);
...
protected:
    ...
    /** checks if the penalty shootout is over */
    void CheckPenaltyShootoutEnd();
    ...
    */
    ...
    /** Move the player out of the given area.
        The player is moved towards the own half.
        \param box the rectangular area to be checked
        \param minDist the minimum distance players will be moved away from box
        \param agentState the agent to check and move.
      */
    void ClearPlayer(const salt::AABB2& box, float minDist, std::shared_ptr<AgentState> agentState);
    ...
    /**
     * Clear the players during penalty shootout.
     * This keeps the idle player away from the active field and assures
     * that the player is not starting too close to the ball.
     */
    void ClearPlayersDuringPenaltyShootout();
    ...
    */
     ...
     */
    void PunishIndirectKickGoal(std::shared_ptr<oxygen::AgentAspect> agent,
                                TTeamIndex scoredOnTeamIdx);

    /**
     * Punish agent's foul (double touch) committed during a kick 
     */
    void PunishFreeKickFoul(std::shared_ptr<oxygen::AgentAspect> agent);

    /** returns true if last kick was from free kick */
    bool WasLastKickFromFreeKick(
        std::shared_ptr<oxygen::AgentAspect> &lastKicker);

    bool MoveAgent(std::shared_ptr<oxygen::Transform> agent_aspect, const salt::Vector3f& pos, bool fSafe=true, bool fAvoidBall=true);
...
protected:
    ...
    std::shared_ptr<oxygen::AgentAspect> mPreLastCollidingAgent;
    ...
    std::shared_ptr<oxygen::AgentAspect> mLastFreeKickTaker;
    ...
    std::shared_ptr<oxygen::AgentAspect> mLastKickOffTaker;
    ...
    /** in penalty shootout: the distance between the ball and the goal */
    float mPenaltyShootoutBallDistance;
    /** in penalty shootout: the distance between the ball and the player */
    float mPenaltyShootoutBehindBallDistance;
    /** in penalty shootout: the time limitation for scoring a goal */
    TTime mPenaltyShootoutMaxTime;
    /** the minimum number of penalty shootout rounds to play */
    int mPenaltyShootoutMinRounds;
    /** the maximum number of penalty shootout rounds to play */
    int mPenaltyShootoutMaxRounds;
    ...
    /** if a kick pass is currently active */
    bool mKickPassActive;

    /** Radius within a player's center which the ball is considered being held*/
    float mBallHoldRadius;
    /** Max time a player can hold the ball */
    float mBallHoldMaxTime;
    /** Max time a goalie can hold the ball while in the penalty area */
    float mBallHoldGoalieMaxTime;
    /** Time since a player stopped holding the ball before ball holding time resets */
    float mBallHoldResetTime;
    /** Max distance a player can hold the ball */
    float mBallHoldMaxDistance;
    /** Distance within an opponent must be from the ball for a foul to be called */
    float mBallHoldOppDistance;
    /** use beaming to penalize ball holding */
    bool mBallHoldBeamPenalty;
    /** Time since player has been holding the ball */
    int playerTimeSinceStartBallHold[12][3];
    /** Time since player has stopped holding the ball */
    int playerTimeSinceStopBallHold[12][3];
    /** Starting position of ball since player last started holding the ball */
    salt::Vector3f playerBallPosAtStartBallHold[12][3];
    ...
    /** the team that is currently kicking in a penalty shootout */
    TTeamIndex mPenaltyShootoutCurrentKickerTeam;
    /** the current penalty shootout kicker */
    std::shared_ptr<oxygen::AgentAspect> mPenaltyShootoutKicker;
    /** if the ball has been placed in a half during penalty shootout */
    bool mPenaltyShootoutBallPlaced;
    /** timestamp marking when a penalty shot (single kick with optional goal) started */
    TTime mPenaltyShootoutStart;
    /** if the kicker is still allowed to touch the ball in a penalty shootout */
    bool mPenaltyShootoutBallTouchOver;

    /** if a goal was already awarded in this cycle */
    bool mGoalAwarded;
    /** the time passed between the last and the current call of "Update" */
    float mDeltaTime;

#ifdef RVDRAW
    std::shared_ptr<RVSender> mRVSender;
#endif // RVDRAW
};
