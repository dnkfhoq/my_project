...
TrainerCommandParser::TrainerCommandParser() : MonitorCmdParser()
{
    ...
    mPlayModeMap[STR_PM_PASS_LEFT] = PM_PASS_LEFT;
    mPlayModeMap[STR_PM_PASS_RIGHT] = PM_PASS_RIGHT;
...
TrainerCommandParser::OnLink()
{
    ...
    // from S-Expressions
    mSexpParser = std::dynamic_pointer_cast<oxygen::BaseParser>(GetCore()->New("SexpParser"));
    ...
    mGameControl = std::dynamic_pointer_cast<GameControlServer>
    ...
    mSimServer = std::dynamic_pointer_cast<SimulationServer>
    ...
    mMonitorControl = std::dynamic_pointer_cast<MonitorControl>
...
void TrainerCommandParser::ParseMonitorMessage(const std::string& data)
{
    ...
    std::shared_ptr<PredicateList> predList = mSexpParser->Parse(data);
...
void TrainerCommandParser::ParseSelectCommand(const oxygen::Predicate & predicate)
{
    ...
    std::shared_ptr<SoccerRuleAspect> soccerRuleAspect;
...
void TrainerCommandParser::ParseReposCommand(const oxygen::Predicate & predicate)
{
    ...
    for (;iter != agentStates.end(); ++iter)
    {
            ...
            (!specified && (*iter)->IsSelected()))
        {
            ...
            std::shared_ptr<RigidBody> ballBody;
