...
void RCS3DMonitor::ParseMonitorMessage(const std::string& data)
{
    ...
    for (
        ...
        {
            std::static_pointer_cast<MonitorCmdParser>(*iter)
...
void RCS3DMonitor::DescribeBall(stringstream& ss, NodeCache& entry, std::shared_ptr<Ball> ball)
{
      if (mFullState)
          {
              ss << "(nd Ball";
  ...
  DescribeTransform(ss, entry, std::static_pointer_cast<Transform>(ball), false);
}
...
void RCS3DMonitor::DescribeLight(stringstream& ss, std::shared_ptr<Light> light)
{
    if (! mFullState)
        {
...
void RCS3DMonitor::DescribeTransform(stringstream& ss, NodeCache& entry, std::shared_ptr<Transform> transform, bool prefix)
{
    if (prefix)
        {
            if (mFullState)
...
void RCS3DMonitor::DescribeMesh(stringstream& ss, std::shared_ptr<StaticMesh> mesh)
{
    std::shared_ptr<SingleMatNode> singleMat =
        std::dynamic_pointer_cast<SingleMatNode>(mesh);
    ...
    if (singleMat.get() != 0)
        {
            std::shared_ptr<Material> mat = singleMat->GetMaterial();
            if (mat.get() != 0)
                {
                    ss << " (sMat " << mat->GetName() << ")";
                }
...
RCS3DMonitor::NodeCache* RCS3DMonitor::LookupNode(std::shared_ptr<BaseNode> node)
{
    if (node.get() == 0)
        {
            assert(false);
    ...
    std::shared_ptr<Ball> ball
        = std::dynamic_pointer_cast<Ball>(node);
    if (ball.get() != 0)
        {
            mNodeCache[node]
    ...
    std::shared_ptr<Transform> transform
        = std::dynamic_pointer_cast<Transform>(node);
    if (transform.get() != 0)
        {
            mNodeCache[node]
    ...
    std::shared_ptr<StaticMesh> mesh
        = std::dynamic_pointer_cast<StaticMesh>(node);
    if (mesh.get() != 0)
        {
    ...
    std::shared_ptr<Light> light
        = std::dynamic_pointer_cast<Light>(node);
    if (light.get() != 0)
        {
...
bool RCS3DMonitor::DescribeNode(stringstream& ss, std::shared_ptr<BaseNode> node)
{
    ...
    switch (entry->type)
        ...
        case NT_BALL:
            DescribeBall
                (ss, (*entry), std::static_pointer_cast<Ball>(node));
            return true;

        case NT_TRANSFORM:
            DescribeTransform
                (ss, (*entry), std::static_pointer_cast<Transform>(node));
            return true;

        case NT_STATICMESH:
            DescribeMesh
                (ss, std::static_pointer_cast<StaticMesh>(node));
            return true;

        case NT_LIGHT:
            DescribeLight
                (ss, std::static_pointer_cast<Light>(node));
            return true;
        }
}
...
void RCS3DMonitor::DescribeScene(stringstream& ss, std::shared_ptr<BaseNode> node)
{
    ...
    for (TLeafList::iterator i = baseNodes.begin(); i!= baseNodes.end(); ++i)
        {
            std::shared_ptr<BaseNode> baseNode = std::dynamic_pointer_cast<BaseNode>(*i);
