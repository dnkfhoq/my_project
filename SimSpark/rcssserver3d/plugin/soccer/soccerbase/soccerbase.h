...
{
public:
    typedef std::vector<std::shared_ptr<AgentState> > TAgentStateList;
    typedef std::map<int, std::shared_ptr<AgentState> > TAgentStateMap;
...
public:
    ...
    /** returns a reference to the AgentState node below the given
        transform node */
    static bool
    GetAgentState(const std::shared_ptr<oxygen::Transform> transform,
    ...
    GetAgentState(const zeitgeist::Leaf& base, TTeamIndex idx,
                 int unum, std::shared_ptr<AgentState>& agent_state);
    ...
    GetGameControlServer(const zeitgeist::Leaf& base,
                         std::shared_ptr<oxygen::GameControlServer>& game_control_server);

    /** returns a reference to the active scene from the SceneServer */
    static bool
    GetActiveScene(const zeitgeist::Leaf& base,
                   std::shared_ptr<oxygen::Scene>& active_scene);

    /** returns a reference to the Ball node */
    static bool
    GetBall(const zeitgeist::Leaf& base, std::shared_ptr<Ball>& ball);
    ...
    /** returns a reference to a ControlAspect registered to the
        GameControlServer */
    static std::shared_ptr<oxygen::ControlAspect>
    ...
    static bool
    MoveAgent(std::shared_ptr<oxygen::Transform> agent_aspect, const salt::Vector3f& pos);

    /* move an agent including all its connected bodies and rotate
       specified angle (DEG) around the z-axis
    */
    static bool
    MoveAndRotateAgent(std::shared_ptr<oxygen::Transform> agent_aspect,
