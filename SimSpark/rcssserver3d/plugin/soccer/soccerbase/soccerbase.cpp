...
SoccerBase::GetSceneServer(const Leaf& base,
                           std::shared_ptr<SceneServer>& scene_server)
{
    scene_server = std::static_pointer_cast<SceneServer>
...
{
    transform_parent = std::dynamic_pointer_cast<Transform>
...
bool
SoccerBase::GetAgentState(const std::shared_ptr<Transform> transform,
                          std::shared_ptr<AgentState>& agent_state)
{
    agent_state =
        std::dynamic_pointer_cast<AgentState>(transform->GetChild("AgentState", true));
...
bool
SoccerBase::GetAgentBody(const std::shared_ptr<Transform> transform,
...
SoccerBase::GetAgentBody(const Leaf& base, TTeamIndex idx,
                         int unum, std::shared_ptr<RigidBody>& agent_body)
{
...
SoccerBase::GetAgentState(const Leaf& base, TTeamIndex idx,
                          int unum, std::shared_ptr<AgentState>& agentState)
{
...
SoccerBase::GetAgentStates(const zeitgeist::Leaf& base,
                           ...
                           TTeamIndex idx)
{
    static std::shared_ptr<GameControlServer> gameCtrl;
    ...
    for (
        ...
        {
            agentState = std::dynamic_pointer_cast<AgentState>((*iter)->GetChild("AgentState", true));
...
SoccerBase::GetGameState(const Leaf& base,
                         std::shared_ptr<GameStateAspect>& game_state)
{
    game_state = std::dynamic_pointer_cast<GameStateAspect>
...
SoccerBase::GetSoccerRuleAspect(const Leaf& base,
                                std::shared_ptr<SoccerRuleAspect> & soccer_rule_aspect)
{
    soccer_rule_aspect = std::dynamic_pointer_cast<SoccerRuleAspect>
...
SoccerBase::GetGameControlServer(const Leaf& base,
                                std::shared_ptr<GameControlServer> & game_control_server)
{
    static std::shared_ptr<GameControlServer> gameControlServer;

    if (gameControlServer.get() == 0)
    {
        gameControlServer = std::dynamic_pointer_cast<GameControlServer>
...
{
    static std::shared_ptr<SceneServer> sceneServer;
...
bool
SoccerBase::GetBody(const Leaf& base, std::shared_ptr<RigidBody>& body)
{
    std::shared_ptr<Transform> parent;
    if (! GetTransformParent(base,parent))
    {
  ...
  body = std::dynamic_pointer_cast<RigidBody>(parent->FindChildSupportingClass<RigidBody>());
...
bool
SoccerBase::GetBall(const Leaf& base, std::shared_ptr<Ball>& ball)
{
    static std::shared_ptr<Scene> scene;
    static std::shared_ptr<Ball> ballRef;
    ...
    if (ballRef.get() == 0)
    {
        ballRef = std::dynamic_pointer_cast<Ball>
...
bool
SoccerBase::GetBallBody(const Leaf& base, std::shared_ptr<RigidBody>& body)
{
    static std::shared_ptr<Scene> scene;
    static std::shared_ptr<RigidBody> bodyRef;
    ...
    if (bodyRef.get() == 0)
    {
        bodyRef = std::dynamic_pointer_cast<RigidBody>
...
SoccerBase::GetBallCollider(const zeitgeist::Leaf& base,
                std::shared_ptr<oxygen::SphereCollider>& sphere)
{
    static std::shared_ptr<Scene> scene;
    static std::shared_ptr<SphereCollider> sphereRef;
    ...
    if (sphereRef.get() == 0)
    {
        sphereRef = std::dynamic_pointer_cast<SphereCollider>
...
SoccerBase::GetControlAspect(const zeitgeist::Leaf& base,const string& name)
{
  ...
  std::shared_ptr<ControlAspect> aspect = std::dynamic_pointer_cast<ControlAspect>
...
bool
SoccerBase::MoveAgent(std::shared_ptr<Transform> agent_aspect, const Vector3f& pos)
{
    Vector3f agentPos = agent_aspect->GetWorldTransform().Pos();

    std::shared_ptr<Transform> parent = std::dynamic_pointer_cast<Transform>
    ...
    {
        std::shared_ptr<RigidBody> childBody =
            std::dynamic_pointer_cast<RigidBody>(*iter);
...
bool
SoccerBase::MoveAndRotateAgent(std::shared_ptr<Transform> agent_aspect, const Vector3f& pos, float angle)
{
    std::shared_ptr<Transform> parent = std::dynamic_pointer_cast<Transform>
            (agent_aspect->FindParentSupportingClass<Transform>().lock());
...
AABB3 SoccerBase::GetAgentBoundingBox(const Leaf& base)
{
    ...
    std::shared_ptr<Space> parent = base.FindParentSupportingClass<Space>().lock();
