...
BeamEffector::PrePhysicsUpdateInternal(float /*deltaTime*/)
{
    ...
    std::shared_ptr<BeamAction> beamAction =
        std::dynamic_pointer_cast<BeamAction>(mAction);
   ...
   mAction.reset();
        ...
        || mGameState->GetPlayMode() == PM_Goal_Right)
    {
        ...
        std::shared_ptr<Transform> agentAspect;
...
BeamEffector::GetActionObject(const Predicate& predicate)
{
  ...
  return std::shared_ptr<ActionObject>(new BeamAction(GetPredicate(), posX, posY, angle));
}
