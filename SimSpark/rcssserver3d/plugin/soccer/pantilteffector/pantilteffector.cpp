...
PanTiltEffector::PrePhysicsUpdateInternal(float /*deltaTime*/)
{
    ...
    std::shared_ptr<PanTiltAction> panTiltAction =
        std::dynamic_pointer_cast<PanTiltAction>(mAction);
    mAction.reset();
    ...
    std::shared_ptr<RestrictedVisionPerceptor> rvp =
...
PanTiltEffector::GetActionObject(const Predicate& predicate)
{
    ...
    return std::shared_ptr<ActionObject>(new PanTiltAction(GetPredicate(),pan,tilt));
}
