...
void
SexpMonitor::AddAgents(std::shared_ptr<Scene> activeScene, std::ostringstream& ss) const
{
    TLeafList nodes;
    ...
    {
        std::shared_ptr<AgentAspect> aspect = std::static_pointer_cast<AgentAspect>(*i);
        ...
        std::shared_ptr<SayEffector> sayEff = std::static_pointer_cast<SayEffector>
...
void
SexpMonitor::AddFlags(std::shared_ptr<Scene> activeScene, std::ostringstream& ss)
{
    ...
    for (TLeafList::iterator i = nodes.begin(); i != nodes.end(); ++i)
    {
        std::shared_ptr<FieldFlag> flag = std::static_pointer_cast<FieldFlag>(*i);
        const salt::Vector3f& pos = flag->GetWorldTransform().Pos();
        std::shared_ptr<ObjectState> state = std::dynamic_pointer_cast<ObjectState>
...
void
SexpMonitor::AddBall(std::shared_ptr<Scene> activeScene, std::ostringstream& ss) const
{
    std::shared_ptr<Ball> ball = std::static_pointer_cast<Ball>(activeScene->GetChild("Ball"));
...
SexpMonitor::GetMonitorInformation(const oxygen::PredicateList& pList)
{
    ...
    std::shared_ptr<Scene> activeScene = sceneServer->GetActiveScene();
...
SexpMonitor::OnLink()
{
    ...
    mCommandParser = std::dynamic_pointer_cast<TrainerCommandParser>
...
{
    mBallState = std::dynamic_pointer_cast<BallStateAspect>
