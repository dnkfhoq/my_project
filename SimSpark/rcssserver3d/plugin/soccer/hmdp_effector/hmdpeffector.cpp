...
void HMDPEffector::PrePhysicsUpdateInternal(float deltaTime)
{
    ...
    std::shared_ptr<HMDPAction> hMDPAction = std::dynamic_pointer_cast<HMDPAction>(mAction);
    mAction.reset();
    if (hMDPAction.get() == 0)
    {
        GetLog()->Error()
...
std::shared_ptr<ActionObject> HMDPEffector::GetActionObject(const Predicate& predicate)
{
    ...
    return std::shared_ptr<ActionObject>(new HMDPAction(GetPredicate(), inMessage));
}
...
void HMDPEffector::OnLink()
{
    ...
    std::shared_ptr<Node> parent = GetParent().lock();
    ...
    // Body-child
    mBody = std::dynamic_pointer_cast<RigidBody>(parent->GetChildOfClass("RigidBody"));
...
void HMDPEffector::ReadOutJointList()
{
    ...
    std::shared_ptr<Node> parent = GetParent().lock();
    std::shared_ptr<Node> grandparent = parent->GetParent().lock();
    ...
    { // set all to yzero (necessary ?)
        ...
        std::shared_ptr<Leaf> join = *j_it;
        std::shared_ptr<BaseNode> jparent =
            std::dynamic_pointer_cast<BaseNode>(join->GetParent().lock());
...
void HMDPEffector::controlPosServo() // for controlling servos
{
    ...
    for (TLeafList::iterator j_it = jointList.begin(); j_it != jointList.end(); j_it++)
    {
        ...
        std::shared_ptr<HingeJoint> joint = std::static_pointer_cast<HingeJoint> (*j_it);
        ...
        if (abs(err) > 0.00001)
        {
            std::shared_ptr<RigidBody> body = joint->GetBody(Joint::BI_FIRST);
