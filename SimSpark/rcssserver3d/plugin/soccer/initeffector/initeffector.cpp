...
InitEffector::PrePhysicsUpdateInternal(float /*deltaTime*/)
{
    ...
    std::shared_ptr<InitAction> initAction =
        std::static_pointer_cast<InitAction>(mAction);
    mAction.reset();
    ...
    for (iter;
        ...
        {    
            std::shared_ptr<Body> childBody =
                std::dynamic_pointer_cast<Body>(*iter);
...
InitEffector::GetActionObject(const Predicate& predicate)
{
    ...
    return std::shared_ptr<ActionObject>(new InitAction(GetPredicate(),name,unum));
}
