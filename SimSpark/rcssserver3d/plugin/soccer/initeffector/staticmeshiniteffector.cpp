...
StaticMeshInitEffector::PrePhysicsUpdateInternal(float deltaTime)
{
    ...
    std::shared_ptr<AgentState> state = static_pointer_cast<AgentState>
    ...
    string unumMat = "matNum" + std::to_string(unum);
    
    // get parent of the agent aspect
    std::shared_ptr<Node> parent = dynamic_pointer_cast<Node>
    ...
    {
        std::shared_ptr<StaticMesh> m = static_pointer_cast<StaticMesh>(*iter);
