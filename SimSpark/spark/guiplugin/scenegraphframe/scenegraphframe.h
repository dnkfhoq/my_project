...
private:
    ...
    */
    std::shared_ptr<SparkController> getSparkController(std::shared_ptr<SimulationTask> task);
    /*!
     \brief Returns pointer to the currently selected task or 0-pointer if nothing is selected.
    */
    std::shared_ptr<SimulationTask> getCurrentTask();
    /*!
    ...
    */
    void observeSparkSimulationThread(std::shared_ptr<SparkSimulationThread> thread);
    /*!
    ...
    */
    bool checkAndInsertTask(std::shared_ptr<SimulationTask> task, int localIndex);
...
private:
    ...
    std::vector<std::pair<int, std::shared_ptr<SimulationTask> > > mTasks; /*!< List of all tasks shown with their id and a pointer to the task. */
