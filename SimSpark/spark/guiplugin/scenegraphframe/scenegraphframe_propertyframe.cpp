...
void PropertyFrame::inspectLeaf(std::shared_ptr<zeitgeist::Leaf> leaf)
{
...
void PropertyFrame::init(SceneGraphFrame* creator)
{
    ...
    std::shared_ptr<ServerThread> thread = std::shared_ptr<ServerThread>(mCreator->getCarbon()->getSimulationManager()->getSparkServerThread());
    if (thread.get() == 0)
    {
    ...
    std::shared_ptr<SparkController> controller = std::shared_ptr<SparkController>(thread->getSparkController());
    if (controller.get() == 0)
    {
