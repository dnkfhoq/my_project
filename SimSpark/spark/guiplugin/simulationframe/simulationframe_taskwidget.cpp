...
TaskWidget::TaskWidget(SimulationFrame* parent, std::shared_ptr<SimulationTask> task) :
    mTask(task),
    mParent(parent)
{
    //--------------------
...
TaskDefinition& TaskWidget::getTaskDefinition(std::shared_ptr<SimulationTask> task)
{
...
{
    if(std::shared_ptr<SimulationTask> task = mTask.lock())
    {
...
void TaskWidget::changeListEntry(QListWidgetItem* item, QListWidget* listWidget, QStringList& list, int& chosenEntry, bool first)
{
    ...
    std::shared_ptr<SimulationTask> task = mTask.lock();
    if (task.get() == 0)
    {
        updateDisplay();
        return;
...
void TaskWidget::deleteTask()          
{
    ...
    if(std::shared_ptr<SimulationTask> sptr = mTask.lock())
    {
        task = &*sptr;
    }
    else
