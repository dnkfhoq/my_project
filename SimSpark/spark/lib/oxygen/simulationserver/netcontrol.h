...
public:
    ...
    public:
        int id;
        rcss::net::Addr addr;
        std::shared_ptr<rcss::net::Socket> socket;

    public:
        Client() : id(-1) {};
        Client(int i, const rcss::net::Addr& a,
               std::shared_ptr<rcss::net::Socket> s =
               std::shared_ptr<rcss::net::Socket>()
               ) : id(i), addr(a), socket(s) {};
    };

    // mapping from a client remote address to Client struct
    typedef std::map<rcss::net::Addr, std::shared_ptr<Client> > TAddrMap;

    // mapping from a client remote address to a corresponding buffer
    typedef std::map<rcss::net::Addr, std::shared_ptr<NetBuffer> > TBufferMap;
...
public:
    ...
    void SendClientMessage(std::shared_ptr<Client> client,
    ...
    static std::shared_ptr<rcss::net::Socket>
...
protected:
    ...
    std::shared_ptr<NetMessage> mNetMessage;
    ...
    std::shared_ptr<rcss::net::Socket> mSocket;
