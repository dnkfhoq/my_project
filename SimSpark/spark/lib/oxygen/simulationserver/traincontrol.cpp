...
void TrainControl::ActAgent()
{
        ...
        )
    {
        ...
        string cmd("");
        std::shared_ptr<Behavior> behavior = client->behavior;
        if (behavior.get() != 0)
        {
...
void TrainControl::ClientConnect(std::shared_ptr<Behavior> behavior)
{
    if (
        (mGameControlServer.get() == 0) ||
        (behavior.get() == 0)
...
void TrainControl::ClientDisconnect(std::shared_ptr<Behavior> behavior)
{
    if (
        (mGameControlServer.get() == 0) ||
        (behavior.get() == 0)
