...
{
    mBufferSize = 64 * 1024;
    mBuffer     = std::shared_ptr<char[]>(new char[mBufferSize]);
    mSocketType = ST_TCP;
...
std::shared_ptr<Socket> NetControl::CreateSocket(ESocketType type)
{
    ...
    try
        {
            switch (type)
                {
                case ST_UDP:
                    socket = std::shared_ptr<Socket>(new UDPSocket());
                    break;

                case ST_TCP:
                    socket = std::shared_ptr<Socket>(new TCPSocket());
                    break;
...
void NetControl::AddClient(const Addr& from, std::shared_ptr<Socket> socket)
{
    std::shared_ptr<Client> client(new Client(mClientId,from,socket));
...
void NetControl::RemoveClient(TAddrMap::iterator iter)
{
    ...
    std::shared_ptr<Socket> socket = client->socket;
...
void NetControl::ClientConnect(std::shared_ptr<Client> /*client*/)
{
    // empty callback, implemented in derived classes
}

void NetControl::ClientDisconnect(std::shared_ptr<Client> /*client*/)
{
    // empty callback, implemented in derived classes
}

void NetControl::SendClientMessage(std::shared_ptr<Client> client, const string& msg)
{
    if (client.get() == 0)
        {
            return;
    ...
    for(;;)
        {
            ...
            try
                {
                    Addr addr;
                    std::shared_ptr<Socket> socket(mSocket->accept(addr));
