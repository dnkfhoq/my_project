...
{
    mParser = std::dynamic_pointer_cast<BaseParser>(GetCore()->New(parserName));
...
{
    std::shared_ptr<ControlAspect> aspect
        = std::dynamic_pointer_cast<ControlAspect>(GetCore()->New(aspectName));
...
GameControlServer::AgentConnect(int id)
{
    ...
    std::shared_ptr<AgentAspect> aspect = std::dynamic_pointer_cast<AgentAspect>
...
std::shared_ptr<ActionObject::TList>
GameControlServer::Parse(int id, const string& str) const
{
    ...
    if (iter == mAgentMap.end())
    {
        GetLog()->Error()
            ...
            << id << "\n";
        return std::shared_ptr<ActionObject::TList>();
    }
    ...
    std::shared_ptr<PredicateList> predicates(mParser->Parse(str));

    // construct an ActionList using the registered effectors
    std::shared_ptr<ActionObject::TList> actionList(new ActionObject::TList());

    std::shared_ptr<AgentAspect> aspect = (*iter).second;
            ...
            )
    {
        ...
        std::shared_ptr<Effector> effector = aspect->GetEffector(predicate.name);
        if (effector.get() == 0)
        {
            GetLog()->Warning()
        ...
        std::shared_ptr<ActionObject> action(effector->GetActionObject(predicate));
...
std::shared_ptr<AgentAspect>
GameControlServer::GetAgentAspect(int id)
{
    TAgentMap::iterator iter = mAgentMap.find(id);
    if (iter == mAgentMap.end())
    {
        return std::shared_ptr<AgentAspect>();
    }
...
GameControlServer::Update(float deltaTime)
{
    ...
    {
        std::shared_ptr<ControlAspect> aspect =
            std::static_pointer_cast<ControlAspect>(*iter);
