...
public:
    ...
    typedef std::list<std::shared_ptr<AgentAspect> > TAgentAspectList;
...
public:
    ...
    */
    ...
    /** returns the parser currently registered to the
        GameControlServer
    */
    std::shared_ptr<BaseParser> GetParser();
    ...
    */
    std::shared_ptr<ActionObject::TList> Parse
    ...
    */
    ...
    std::shared_ptr<AgentAspect> GetAgentAspect(int id);
...
protected:
    typedef std::map<int, std::shared_ptr<AgentAspect> > TAgentMap;

    /** a map from agent IDs to agent instances */
    TAgentMap mAgentMap;

    /** the parser instance used */
    std::shared_ptr<BaseParser> mParser;
