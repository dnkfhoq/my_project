...
public:
    ...
    std::shared_ptr<Scene> GetActiveScene() { return mActiveScene.get(); }
...
protected:
    ...
    /** reparents all children of node to the parent of node; Their
        ...
        matrix of node
     */
    void ReparentTransformChildren(std::shared_ptr<Transform> node);

    /** recursively reparents all transform nodes whose only children
        are also transform node
     */
    void RemoveTransformPaths(std::shared_ptr<zeitgeist::Leaf> root);
