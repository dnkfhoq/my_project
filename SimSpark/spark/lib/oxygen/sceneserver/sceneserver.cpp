...
{
    std::shared_ptr<CoreContext> context = GetCore()->CreateContext();

    mPhysicsServer = std::static_pointer_cast<PhysicsServer>
...
bool SceneServer::CreateScene(const std::string &location)
{
    ...
    std::shared_ptr<Scene> scene = std::static_pointer_cast<Scene>
...
bool SceneServer::ImportScene(const string& fileName, std::shared_ptr<BaseNode> root,
    ...
    for (
         ...
         )
        {
            std::shared_ptr<SceneImporter> importer =
                std::static_pointer_cast<SceneImporter>(*iter);
            ...
            if (importer->ImportScene(file,root,parameter))
                {
                    ...
                    std::shared_ptr<Scene> scene = root->GetScene();
                    if (scene.get() != 0)
                        {
...
void SceneServer::ReparentTransformChildren(std::shared_ptr<Transform> node)
{
    std::shared_ptr<BaseNode> parent =
        std::dynamic_pointer_cast<BaseNode>(node->GetParent().lock());

    // while not empty
    while (node->begin() != node->end())
        {
            std::shared_ptr<Leaf> child = (*node->begin());

            std::shared_ptr<Transform> tChild
...
void SceneServer::RemoveTransformPaths(std::shared_ptr<Leaf> root)
{
    if (root.get() == 0)
        {
            return;
    ...
    std::shared_ptr<Transform> trans = std::dynamic_pointer_cast<Transform>(root);
    ...
    while (iter != root->end())
        {
            std::shared_ptr<Transform> tChild = std::dynamic_pointer_cast<Transform>(*iter);
            if (tChild.get() == 0)
                {
                    tChildOnly = false;
                } else
...
{
    std::shared_ptr<SceneImporter> importer
        = std::dynamic_pointer_cast<SceneImporter>(GetCore()->New(importerName));
