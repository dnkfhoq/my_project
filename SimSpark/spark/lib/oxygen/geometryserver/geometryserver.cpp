...
{
    std::shared_ptr<MeshImporter> importer
        = std::dynamic_pointer_cast<MeshImporter>(GetCore()->New(importerName));
...
GeometryServer::GetMesh(const string& name, const::ParameterList& parameter)
{
    ...
    {
        std::shared_ptr<MeshImporter> importer =
            std::static_pointer_cast<MeshImporter>(*iter);
         ...
         )
    {
        ...
        std::shared_ptr<TriMesh> mesh = importer->ImportMesh(name,parameter);
...
void
GeometryServer::RegisterMesh(std::shared_ptr<TriMesh> mesh)
{
    if (mesh.get() == 0)
    {
        GetLog()->Debug()
    ...
    for (TLeafList::const_iterator bi = exporters.begin(); bi != exporters.end(); ++bi)
    {
        ...
        std::shared_ptr<MeshExporter> mb = std::static_pointer_cast<MeshExporter>(*bi);
...
{
    std::shared_ptr<MeshExporter> exporter
        = std::dynamic_pointer_cast<MeshExporter>(GetCore()->New(name));
