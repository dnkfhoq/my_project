...
std::shared_ptr<TriMesh> StdMeshImporter::UnitSphereMesh()
{
    std::shared_ptr<TriMesh> mesh(new TriMesh());

    std::shared_ptr<float[]> pos(new float[gNumSphereVertices*3]);
    ...
    std::shared_ptr<IndexBuffer> idx(new IndexBuffer());
...
std::shared_ptr<TriMesh> StdMeshImporter::UnitBoxMesh()
{
    std::shared_ptr<TriMesh> mesh(new TriMesh());

    std::shared_ptr<float[]> pos(new float[gNumBoxVertices*3]);
    memcpy(pos.get(),gBoxVertices,gNumBoxVertices*3*sizeof(float));
    mesh->SetPos(pos,gNumBoxVertices);

    std::shared_ptr<float[]> normals(new float[gNumBoxVertices*3]);
    memcpy(normals.get(),gBoxNormals,gNumBoxVertices*3*sizeof(float));
    mesh->SetNormals(normals);

    std::shared_ptr<float[]> tex(new float[gNumBoxVertices*3]);
...
std::shared_ptr<TriMesh> StdMeshImporter::UnitCapsule(const ParameterList& parameter)
{
    //
    ...
    std::shared_ptr<float[]> pos(new float[numVertices * 3]);
    std::shared_ptr<float[]> normals(new float[numVertices * 3]);
    float* cVert = pos.get();
...
std::shared_ptr<TriMesh> StdMeshImporter::UnitCylinder()
{
    std::shared_ptr<TriMesh> mesh(new TriMesh());

    std::shared_ptr<float[]> pos(new float[gNumCylinderVertices*3]);
    memcpy(pos.get(),gCylinderVertices,gNumCylinderVertices*3*sizeof(float));
    mesh->SetPos(pos,gNumCylinderVertices);

    std::shared_ptr<float[]> normals(new float[gNumCylinderVertices*3]);
