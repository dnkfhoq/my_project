...
public:
    ...
    {
    public:
        std::shared_ptr<IndexBuffer> indeces;
        std::string material;

    public:
        Face(std::shared_ptr<IndexBuffer> i, std::string m)
...
public:
    ...
    void SetPos(std::shared_ptr<float[]> pos, int vertexCount);

    /** returns the list of vertices */
    const std::shared_ptr<float[]> GetPos() const;

    /** returns the number of stored vertices */
    int GetVertexCount() const;

    /** sets the list of texture coordinates */
    void SetTexCoords(std::shared_ptr<float[]> texCoords);

    /** returns the list of texture coordinates */
    const std::shared_ptr<float[]> GetTexCoords() const;

    /** sets the list of surface normals */
    void SetNormals(std::shared_ptr<float[]> normal);

    /** returns the list of surface normals */
    const std::shared_ptr<float[]> GetNormals() const;

    /** adds a face, i.e. a list of indeces. Each consecutive triple
        describes one triangle */
    void AddFace(std::shared_ptr<IndexBuffer> indeces,
...
protected:
    ...
    std::shared_ptr<float[]> mPos;

    /** the list of texture coordinates */
    std::shared_ptr<float[]> mTexCoords;

    /** the list of surface normals */
    std::shared_ptr<float[]> mNormal;
