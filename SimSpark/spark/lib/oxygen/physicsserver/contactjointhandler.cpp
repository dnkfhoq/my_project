...
std::shared_ptr<ContactJointHandlerInt> ContactJointHandler::mContactJointHandlerImp;
...
void ContactJointHandler::OnLink()
{
    ...
    if (mContactJointHandlerImp.get() == 0)
        mContactJointHandlerImp = std::dynamic_pointer_cast<ContactJointHandlerInt>
...
void
ContactJointHandler::HandleCollision(std::shared_ptr<Collider> collidee, GenericContact& contact)
{
    if (
        (mCollider.get() == 0) ||
        (mWorld.get() == 0) ||
    ...
    std::shared_ptr<ContactJointHandler> handler =
