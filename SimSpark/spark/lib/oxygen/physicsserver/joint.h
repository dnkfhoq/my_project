...
public:
    ...
    */
    virtual void Attach(std::shared_ptr<RigidBody> body1,
    ...
    */
    std::shared_ptr<RigidBody> GetBody(EBodyIndex idx);

    /** returns the Joint node corresponding to the given joint */
    static std::shared_ptr<Joint> GetJoint(long jointID);
    ...
    /** returns true if the two given bodies are connected by a
        joint */
    static bool AreConnected (std::shared_ptr<RigidBody> body1,
                              std::shared_ptr<RigidBody> body2);
     ...
     */
    static bool AreConnectedExcluding (std::shared_ptr<RigidBody> body1,
                                       std::shared_ptr<RigidBody> body2,
                                       int joint_type);
...
protected:
    ...
    std::shared_ptr<RigidBody> GetBody(const std::string& path);
    ...
    std::shared_ptr<GenericJointFeedback> mFeedback;
    ...
    static std::shared_ptr<JointInt> mJointImp;
};
