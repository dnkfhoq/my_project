...
CollisionHandler::UpdateCached()
{
    ...
    mWorld = std::static_pointer_cast<World>(scene->GetChildOfClass("World"));
    if (mWorld.get() == 0)
        {
            GetLog()->Debug()
    ...
    mSpace = std::static_pointer_cast<Space>(scene->GetChildOfClass("Space"));
    if (mSpace.get() == 0)
        {
            GetLog()->Debug()
    ...
    mCollider = std::static_pointer_cast<Collider>(GetParent().lock());
    if (mCollider.get() == 0)
        {
            GetLog()->Debug()
