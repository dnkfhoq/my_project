...
std::shared_ptr<JointInt> Joint::mJointImp;
...
{
    if (mJointImp.get() == 0)
        mJointImp = std::dynamic_pointer_cast<JointInt>
...
std::shared_ptr<Joint> Joint::GetJoint(long jointID)
{
    if (jointID == 0)
        {
    ...
    std::shared_ptr<Joint> joint = std::static_pointer_cast<Joint>
        (jointPtr->GetSelf().lock());

    if (joint.get() == 0)
        {
            // we cannot use the logserver here
            cerr << "ERROR: (Joint) got no std::shared_ptr for dJointID "
                 << jointID << "\n";
        }
...
void Joint::Attach(std::shared_ptr<RigidBody> body1, std::shared_ptr<RigidBody> body2)
{
    if (mJointID == 0)
        {
            GetLog()->Error()
...
std::shared_ptr<RigidBody> Joint::GetBody(const std::string& path)
{
    if (path.empty())
        {
    ...
    std::shared_ptr<Leaf> mySelf = std::static_pointer_cast<Leaf>
        (GetSelf().lock());

    std::shared_ptr<Leaf> leaf = GetCore()->Get(path,mySelf);
    ...
    std::shared_ptr<RigidBody> body = std::dynamic_pointer_cast<RigidBody>(leaf);
...
{
    std::shared_ptr<RigidBody> body1 = GetBody(path1);
    std::shared_ptr<RigidBody> body2 = GetBody(path2);
...
std::shared_ptr<RigidBody> Joint::GetBody(EBodyIndex idx)
{
    long bodyID = mJointImp->GetBodyID(idx, mJointID);
    return RigidBody::GetBody(bodyID);
}

bool Joint::AreConnected(std::shared_ptr<RigidBody> body1, std::shared_ptr<RigidBody> body2)
{
    if (
        (body1.get() == 0) ||
        (body2.get() == 0)
...
bool Joint::AreConnectedExcluding (std::shared_ptr<RigidBody> body1,
