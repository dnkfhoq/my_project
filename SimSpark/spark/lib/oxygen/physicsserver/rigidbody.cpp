...
std::shared_ptr<RigidBodyInt> RigidBody::mRigidBodyImp;
...
bool RigidBody::CreateBody()
{
    ...
    if (mRigidBodyImp.get() == 0)
        mRigidBodyImp = std::dynamic_pointer_cast<RigidBodyInt>
...
void RigidBody::OnLink()
{
    ...
    std::shared_ptr<BaseNode> baseNode = std::static_pointer_cast<BaseNode>
        (GetParent().lock());
...
void RigidBody::PrePhysicsUpdateInternal(float /*deltaTime*/)
{
    ...
    if (mMassTransformed)
    {
        ...
        for (TLeafList::iterator iter = transformColliders.begin(); iter != transformColliders.end(); ++iter)
        {
            ...
            std::shared_ptr<Collider> collider = std::static_pointer_cast<TransformCollider>(*iter)->FindChildSupportingClass<Collider>();
            if (collider.get())
            {
        ...
        {
            std::shared_ptr<Transform> transform = std::dynamic_pointer_cast<Transform>(*iter);
...
std::shared_ptr<RigidBody> RigidBody::GetBody(long id)
{
    long bodyID = (long) id;
    if (bodyID == 0)
        {
    ...
    if (body.get() == 0)
        {
            // we cannot use the logserver here
            cerr << "ERROR: (RigidBody) got no std::shared_ptr for dBodyID "
                 << id << "\n";
        }
...
std::unique_ptr<GenericMass, GenericMassDeleter> RigidBody::CreateMass(float mass, salt::Vector3f cVector)
{
