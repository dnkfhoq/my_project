...
std::shared_ptr<BoxColliderInt> BoxCollider::mBoxColliderImp;
...
bool BoxCollider::ConstructInternal()
{
    if (mBoxColliderImp.get() == 0)
        mBoxColliderImp = std::dynamic_pointer_cast<BoxColliderInt>
...
bool BoxCollider::CheckSeparatingPlane(const Vector3f& RPos, const Vector3f& Plane, std::shared_ptr<BoxCollider> box2, float tol)
{
...
bool BoxCollider::CheckCollisions( std::shared_ptr<BoxCollider> box2, float tol)
{
    Vector3f RPos;
