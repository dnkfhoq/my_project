...
std::shared_ptr<SpaceInt> Space::mSpaceImp;
...
{  
    ...
    std::shared_ptr<Collider> collider = Collider::GetCollider(obj1);
    std::shared_ptr<Collider> collidee = Collider::GetCollider(obj2);
...
{
    if (mSpaceImp.get() == 0)
        mSpaceImp = std::dynamic_pointer_cast<SpaceInt>
