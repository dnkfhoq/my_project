...
std::shared_ptr<PhysicsServerInt> PhysicsServer::mPhysicsServerImp;
...
{ 
    if (mPhysicsServerImp.get() == 0)
        mPhysicsServerImp = std::dynamic_pointer_cast<PhysicsServerInt>
            (GetCore()->New("PhysicsServerImp"));
        
    mPhysicsServerImp->InitEngine();  
}

void PhysicsServer::UpdateCache(std::shared_ptr<Scene> activeScene)
{
    if (mActiveSpace.get() == 0)
        {
            // cache the space reference
            mActiveSpace = std::dynamic_pointer_cast<Space>
    ...
    if (mActiveWorld.get() == 0)
        {
            ...
            mActiveWorld = std::dynamic_pointer_cast<World>
...
void PhysicsServer::SetUpBox(std::shared_ptr<RigidBody> body, string name)
{
    if (body.get() != 0)
        {
            body->SetName(name);
