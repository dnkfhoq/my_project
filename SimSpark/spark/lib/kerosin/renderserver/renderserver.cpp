...
RenderServer::Render(bool clean)
{
    ...
    if (mCamera.get() == 0)
      mCamera = std::static_pointer_cast<Camera>(mActiveScene->GetChildOfClass("Camera", true));
  ...
  else
  {
    ...
    else
    {
        ...
        {
            (std::static_pointer_cast<Light>(*iter))->Prepare();
        }
    }
...
void
RenderServer::RenderScene(std::shared_ptr<BaseNode> node, unsigned pass)
{
    std::shared_ptr<RenderNode> renderNode = std::dynamic_pointer_cast<RenderNode>(node);
    if (renderNode.get() != 0 &&
    ...
    {
        std::shared_ptr<BaseNode> node = std::dynamic_pointer_cast<BaseNode>(*i);
        if (node.get() == 0)
        {
            continue;
        }
...
void
RenderServer::BindCamera(std::shared_ptr<Camera>& camera)
{
    camera->Bind();
...
std::weak_ptr<RenderNode> RenderServer::GetPickedNode() const
{
    return mPickedNode;
}
...
void RenderServer::NextCamera()
{
    ...
    for (TLeafList::iterator iter = cameras.begin(); iter != cameras.end(); ++iter)
        ...
        {
            ++iter;
            mCamera = iter != cameras.end() ? std::static_pointer_cast<Camera>(*iter) : std::static_pointer_cast<Camera>(cameras.front());
            break;
        }
}
...
void RenderServer::PreviousCamera()
{
    ...
    for (TLeafList::iterator iter = cameras.begin(); iter != cameras.end(); ++iter)
        if (*iter == mCamera)
        {
            mCamera = iter != cameras.begin() ? std::static_pointer_cast<Camera>(*(--iter)) : std::static_pointer_cast<Camera>(cameras.back());
            break;
        }
}
