...
RenderControl::RenderCustom()
{
    ...
    {
        std::static_pointer_cast<CustomRender>((*iter))->Render();
    }
}

void
kerosin::RenderControl::InitSimulation()
{
    mOpenGLServer->InitOpenGLSystem();

    TLeafList customRenders;
    ListChildrenSupportingClass<CustomRender>(customRenders);
    for (auto &crender: customRenders)
        static_pointer_cast<CustomRender>(crender)->Init();
}
