...
OpenGLServer::Init(const string& openGLSysName)
{
    ...
    if (! openGLSysName.empty())
    {
        ...
        mGLSystem = std::dynamic_pointer_cast<OpenGLSystem>
...
bool OpenGLServer::InitOpenGLSystem()
{
    if (mGLSystem->Init() == false)
    {
        GetLog()->Error() << "(InputServer) ERROR: unable to initialize the OpenGL System\n";
        return false;
    }
    return true;
}
