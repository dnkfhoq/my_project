...
std::shared_ptr<Texture> TextureServer::GetTexture(const std::string &name)
{
    ...
    std::shared_ptr<Image> image = mImageServer->Load(name.c_str());
    ...
    tex2D->Create(image);
    std::shared_ptr<Texture> texture(tex2D);
