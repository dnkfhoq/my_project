...
bool SoundServer::Init(const std::string &sndSysName)
{
        ...
        mSoundSystem = std::static_pointer_cast<SoundSystem>(GetCore()->New(sndSysName));
...
std::shared_ptr<SoundEffect> SoundServer::LoadEffect(const string& inName)
{
        std::shared_ptr<SoundObject> soundObject;

        if (LoadSoundObject(inName, mEffects, soundObject) == false)
                return std::shared_ptr<SoundEffect>();

        if (soundObject)
        {
                GetLog()->Debug() << "Found a cached sound" << endl;
                return std::static_pointer_cast<SoundEffect>(soundObject);
        }

        // we don't have the sound in the cache, so create it
        std::shared_ptr<SoundEffect> effect(mSoundSystem->CreateEffect(*this));

        // now, we want to load the file from our fileserver
        std::shared_ptr<FileServer>  fileServer = std::static_pointer_cast<FileServer>(GetCore()->Get("/sys/server/file"));
        std::shared_ptr<salt::RFile> file = fileServer->Open(inName.c_str());
        ...
        std::shared_ptr<char[]> buffer(new char[file->Size()]);
...
std::shared_ptr<SoundStream> SoundServer::LoadStream(const string& inName)
{
        GetLog()->Debug() << "SoundServer::LoadStream " << inName << endl;
        std::shared_ptr<SoundObject> soundObject;

        if (LoadSoundObject(inName, mStreams, soundObject) == false)
                return std::shared_ptr<SoundStream>();

        if (soundObject)
        {
                GetLog()->Debug() << "Found a cached sound" << endl;
                return std::static_pointer_cast<SoundStream>(soundObject);
        }

        // we don't have the sound in the cache, so create it
        std::shared_ptr<SoundStream> stream(mSoundSystem->CreateStream(*this));
...
std::shared_ptr<SoundModule> SoundServer::LoadModule(const string& inName)
{
        std::shared_ptr<SoundObject> soundObject;

        if (LoadSoundObject(inName, mModules, soundObject) == false)
                return std::shared_ptr<SoundModule>();

        if (soundObject)
        {
                GetLog()->Debug() << "Found a cached sound" << endl;
                return std::static_pointer_cast<SoundModule>(soundObject);
        }

        // we don't have the sound in the cache, so create it
        std::shared_ptr<SoundModule> module(mSoundSystem->CreateModule(*this));
...
bool SoundServer::LoadSoundObject(const std::string& inName, const TSoundHashMap& map, std::shared_ptr<SoundObject> &soundObject) const
{
        ...
        if (mSoundSystem.get() == NULL)
        {
                ...
                soundObject = std::shared_ptr<SoundObject>();
                return false;
        }
