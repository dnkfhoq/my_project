...
void StaticMesh::RenderInternal()
{
    ...
    while (
        ...
        {
            const std::shared_ptr<Material> material = (*miter);
            ...
            if (material.get() != 0)
                {
                    ...
                    const std::shared_ptr<IndexBuffer>& idx = face.indeces;
...
const std::shared_ptr<TriMesh> StaticMesh::GetMesh()
{
    return mMesh;
}
...
{
    std::shared_ptr<MaterialServer> materialServer =
        std::dynamic_pointer_cast<MaterialServer>(GetCore()->Get("/sys/server/material"));
    ...
    std::shared_ptr<Material> material = materialServer->GetMaterial(newMat);
...
void StaticMesh::ResetMaterials(const std::vector<std::string>& names)
{
    ...
    for (std::vector<std::string>::const_iterator iter = names.begin();
        ...
        std::shared_ptr<Material> material = materialServer->GetMaterial(*iter);
