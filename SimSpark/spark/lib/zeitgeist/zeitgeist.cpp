...
Zeitgeist::~Zeitgeist()
{
    // this Zeitgeist object owns the only std::shared_ptr to the
    ...
    // our std::shared_ptr prior to the call to Core::~Core() and all
...
{
    mCore = std::shared_ptr<Core>(new Core());
...
std::shared_ptr<CoreContext>  Zeitgeist::CreateContext()
{
    return mCore->CreateContext();
}

std::shared_ptr<Core>& Zeitgeist::GetCore()
{
    return mCore;
}
