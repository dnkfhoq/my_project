...
Class::~Class()
{
    if (mInstances.size() > 0)
    {
        ...
        {
            if (std::shared_ptr<Object> j = i->lock())
            {
                cout << "    " << j.get() << endl;
            } else
            {
...
std::shared_ptr<Object> Class::Create()
{
    std::shared_ptr<Object> obj(CreateInstance());

    if (obj.get())
    {
        if (obj->Construct(obj, std::static_pointer_cast<Class>
                           (GetSelf().lock())) == true)
        {
...
std::shared_ptr<Core> Class::GetCore() const
{
    if (mCore.expired())
        {
...
void Class::AttachInstance(const std::weak_ptr<Object> &instance)
{
    mInstances.push_back(instance);
}

void Class::DetachInstance(const std::weak_ptr<Object> &instance)
{
    // mInstances.remove() doesn't work in this case because
    // operator== is not implemented for std::weak_ptr
    ...
    while (first != last)
    {
        ...
        std::shared_ptr<Object> i = first->lock();
        std::shared_ptr<Object> j = instance.lock();
...
void Class::AttachTo(const std::weak_ptr<Core>& core)
{
    mCore = core;
}

void Class::SetBundle(const std::shared_ptr<salt::SharedLibrary> &bundle)
{
    mBundle = bundle;
}
...
Class::TCmdProc Class::GetCmdProc(const std::string &functionName) const
{
    ...
    // class objects
    std::shared_ptr<Leaf> classDir = GetCore()->Get("/classes");
         ...
         )
    {
        ...
        std::shared_ptr<Class> theClass = std::static_pointer_cast<Class>
...
std::shared_ptr<salt::SharedLibrary> Class::GetBundle() const
{
    return mBundle;
}
