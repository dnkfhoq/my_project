...
bool Object::Construct(const std::shared_ptr<Object>& self, const std::shared_ptr<Class>& creator)
{
    mSelf       = self;
    mClass      = creator;
...
std::shared_ptr<Class> Object::GetClass() const
{
    if (mClass.get() == 0)
    {
...
const std::weak_ptr<Object>& Object::GetSelf() const
{
    return mSelf;
}

std::shared_ptr<Core> Object::GetCore() const
{
    assert(mClass.get() != NULL);
    std::shared_ptr<Core> core = mClass->GetCore();
...
void Object::Dump() const
{
    ...
    if (std::shared_ptr<Object> self = mSelf.lock())
    {
        cout << " " << (void*) self.get();
    } else {
        cout << " " << "(expired)";
...
void Object::Invoke(const std::string &functionName, const ParameterList& parameter)
{
    ...
    if (cmd != NULL)
      {
        if (std::shared_ptr<Object> self = GetSelf().lock())
          {
...
const std::shared_ptr<FileServer>& Object::GetFile() const
{
    return GetCore()->GetFileServer();
}

const std::shared_ptr<LogServer>& Object::GetLog() const
{
    return GetCore()->GetLogServer();
}

const std::shared_ptr<ScriptServer>& Object::GetScript() const
{
