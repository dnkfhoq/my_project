...
public:
    ...
    virtual std::shared_ptr<Leaf> GetChild(const std::string &name, bool recursive = false);
    ...
    virtual std::shared_ptr<Leaf> GetChildOfClass(const std::string &name, bool recursive = false);
    ...
    virtual std::shared_ptr<Leaf> GetChildSupportingClass(const std::string &name, bool recursive = false);
    ...
    virtual void RemoveChildReference(const std::shared_ptr<Leaf> &base);
    ...
    virtual bool AddChildReference(const std::shared_ptr<Leaf> &base);
...
Leaf::FindParentSupportingClass() const
{
    ...
    while (node.get() != 0)
    {
        std::shared_ptr<CLASS> test =
            std::dynamic_pointer_cast<CLASS>(node);
        ...
        //node = std::static_pointer_cast<Node>(node->GetParent().lock());
        node = std::static_pointer_cast<Node>(node->GetParent().lock());
