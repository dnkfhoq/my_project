...
std::weak_ptr<Node>& Leaf::GetParent()
{
    return mParent;
}

const std::weak_ptr<Node>& Leaf::GetParent() const
{
    return mParent;
}

std::shared_ptr<Leaf> Leaf::GetChild(const std::string &name, bool /*recursive*/)
{
    if (name.compare("..") == 0)
    {
    ...
    if (name.compare(".") == 0)
    {
        ...
        return std::static_pointer_cast<Leaf>(GetSelf().lock());
    }

    return std::shared_ptr<Leaf>();
}

std::shared_ptr<Leaf> Leaf::GetChildOfClass(const std::string &/*name*/, bool /*recursive*/)
{
    return std::shared_ptr<Leaf>();
}

std::shared_ptr<Leaf> Leaf::GetChildSupportingClass(const std::string &/*name*/, bool /*recursive*/)
{
    return std::shared_ptr<Leaf>();
}
...
void Leaf::GetChildren(const std::string &name, TLeafList &baseList, bool /*recursive*/)
{
    ...
    if (name.compare(".") == 0)
    {
        baseList.push_back(std::static_pointer_cast<Leaf>(GetSelf().lock()));
    }
}
...
Leaf::GetParentSupportingClass(const std::string &name) const
{
  std::shared_ptr<Node> node
    = std::static_pointer_cast<Node>(GetParent().lock());
...
void Leaf::RemoveChildReference(const std::shared_ptr<Leaf> &/*base*/)
{
}

bool Leaf::AddChildReference(const std::shared_ptr<Leaf> &/*base*/)
{
    return false;
}
...
const std::string& Leaf::GetFullPath() const
{
    ...
    if (mCachedFullPath == NULL)
    {
        ...
        if (std::shared_ptr<Leaf> p = GetParent().lock())
        {
            if (p)
            {
                std::shared_ptr<Leaf> blah = GetParent().lock();
...
void Leaf::SetParent(const std::shared_ptr<Node> &newParent)
{
    std::shared_ptr<Node> oldParent = GetParent().lock();
    std::shared_ptr<Leaf> self = std::static_pointer_cast<Leaf>(GetSelf().lock());
    ...
    std::shared_ptr<Leaf> sibling = newParent->GetChild(mName);
