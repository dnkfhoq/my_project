...
{
    mSpark = std::shared_ptr<SimSpark>();
...
SparkController::~SparkController()
{
    std::shared_ptr<GuiSimControl> control(mSimControl.lock());
...
std::shared_ptr<SimSpark> SparkController::getSpark() const
{
    return mSpark;
}

std::weak_ptr<GuiSimControl> SparkController::getSimulationControl()
{
    return mSimControl;
}
    ...
    const QStringList& parameters, const std::vector<std::string>& resourceLocations, bool restart, const QString& logSourceName)
{
    ...
    else
    {
        ...
        mSpark = std::shared_ptr<SimSpark>(new SimSpark(absSecondaryPath.toStdString(), secondaryScriptName.toStdString(), absFilePath.toStdString(), fullPathList));
    }
    ...
    if (!reset)
    {
        ...
        std::shared_ptr<zeitgeist::Leaf> leaf(
            mSpark->GetCore()->CreateContext()->New("gui/GuiSimControl", "sys/server/simulation/GuiControl"));
        mSimControl = std::weak_ptr<GuiSimControl>(std::dynamic_pointer_cast<GuiSimControl>(leaf));
            
        //Init Command queue
        initCommandQueue();

        //Gui uses scene dictionary
        std::shared_ptr<zeitgeist::ScriptServer> server(mSpark->GetCore()->GetScriptServer());
        if (server.get())
        {
...
{
    return queueCommand(std::shared_ptr<SparkCommand::Command>(command), timeout, callback, member, log_error);
}

bool SparkController::queueCommand(int commandId, SparkCommand::Command* command, int timeout, 
    const QObject* callback, const char* member, bool log_error)
{
    return queueCommand(commandId, std::shared_ptr<SparkCommand::Command>(command), timeout, callback, member, log_error);
}

bool SparkController::queueCommand(std::shared_ptr<SparkCommand::Command> command, int timeout, 
    const QObject* callback, const char* member, bool log_error)
{
    if (command.get() == 0)
        return false;
...
bool SparkController::queueCommand(int commandId, std::shared_ptr<SparkCommand::Command> command, int timeout, 
    const QObject* callback, const char* member, bool log_error)
{
    if (command.get() == 0)
        return false;
...
bool SparkController::runScript(const QString& path)
{
    ...
    SimulationManager* manager = Carbon::get()->getSimulationManager();
    std::shared_ptr<zeitgeist::ScriptServer> scriptserver(mSpark->GetCore()->GetScriptServer());
...
void SparkController::addLogStream(Logger::LogLevel level, int sourceId)
{
    ...
    //Add the stream
    mLogStreams.push_back(std::shared_ptr<LogStream>(new LogStream(level, 1024, sourceId)));
...
{    
    ...
    mCommandQueue = std::shared_ptr<SparkCommand::CommandQueue>(new SparkCommand::CommandQueue(this));
