...
PropertyWidget::PropertyWidget(QWidget* parent, std::shared_ptr<SparkController> sparkcontroller) :
    QFrame(parent),
...
{
    std::shared_ptr<SimSpark> spark = getSpark();
    if (spark.get() == 0)
    {
        return;
    }
...
void PropertyWidget::setLeaf(std::weak_ptr<Leaf> leaf)
{
    std::shared_ptr<SimSpark> spark = getSpark();
    if (spark.get() == 0)
        return;
...
{
    const std::shared_ptr<const PropertyManager> manager(mPropList.getPropertyManager());
...
void PropertyWidget::refreshEditFrame()
{
    ...
    std::shared_ptr<SparkProperty::Property> prop = 
        (mSelectedIndex == -1 ? std::shared_ptr<SparkProperty::Property> () : mPropList.getProperty(mSelectedIndex));
    ...
    {   
        ...
        switch (mEditWidgetDataType)
        {
            ...
            case SparkProperty::DT_BOOL  : //no edit window for bool
                break;
            case SparkProperty::DT_VEC3F : 
                vec3 = &std::static_pointer_cast<const DVector3f>(prop->getCurrentValue())->mValue;
                ((Vector3Widget*)mPropertyEditWidget)->setVector(vec3->x(), vec3->y(), vec3->z());
                break;
            case SparkProperty::DT_RGBA  : 
                rgba = &std::static_pointer_cast<const DRGBA>(prop->getCurrentValue())->mValue;
                ((Vector4Widget*)mPropertyEditWidget)->setVector(rgba->r(), rgba->g(), rgba->b(), rgba->a());
                break;
            case SparkProperty::DT_AABB3 : 
                aabb3 = &std::static_pointer_cast<const DAABB3>(prop->getCurrentValue())->mValue;
                ...
                break;
            case SparkProperty::DT_MAT4X4:
                mat = &std::static_pointer_cast<const DMat4x4>(prop->getCurrentValue())->mValue;
...
std::shared_ptr<SimSpark> PropertyWidget::getSpark()
{
    if (mSparkController.get() == 0)
    {
        LOG_ERROR() << "No spark controller";
        return std::shared_ptr<SimSpark>();
    }
...
void PropertyWidget::updateFromEditWidget()
{
    ...
    std::shared_ptr<Property> selectedProperty(mPropList.getProperty(mSelectedIndex));
...
void PropertyWidget::tableItemChanged(QTableWidgetItem* item)
{
    ...
    std::shared_ptr<SparkProperty::Property> prop = mPropList.getProperty(item->row());
    if (prop.get() == 0)
    {
...
{
    std::shared_ptr<Property> prop(mPropList.getProperty(index));
