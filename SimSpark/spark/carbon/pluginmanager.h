...
public:
    ...
    */
    std::shared_ptr<PluginTableModel> getTableModel();
    ...
    */
    ...
    /*!
     ...
     \return pointer to new plugin, or 0 if an error occurred
    */
    AbstractPlugin* createPlugin(std::shared_ptr<PluginDefinition> definition, bool addDefToSimulation = false);
    ...
    */
    bool loadData(std::shared_ptr<Settings> data);
    /*!
     \brief Calls loadData() for all instantiated plugins with the given data object.
     ...
     \return true if anything was loaded
    */
    bool saveData(std::shared_ptr<Settings> data);
...
private:
    ...
    std::shared_ptr<SimulationSetup> mExtensions; /*!< List of plugins that have to be loaded with every simulation. (contains no tasks) */
