    ...
    mSimulation(new Simulation())
{
    ...
    mClosed = false;
    mServerThread = std::shared_ptr<ServerThread>();
    mNextTaskId = 1;
...
std::shared_ptr<ServerThread> SimulationManager::getSparkServerThread() const
{
    return SimulationManager::mServerThread;
}

std::shared_ptr<ServerThread> SimulationManager::initSparkServerThread(ServerThread* server)
{
    ...
    {
        mServerThread = std::shared_ptr<ServerThread>(server);
    }
...
void SimulationManager::loadSimulationSetups()
{
    ...
    {
        std::shared_ptr<SimulationSetup> loadedSetup = loadSimulationSetup(*it, true); //and store it
        if (!loadedSetup.get())
        {
...
std::shared_ptr<SimulationSetup> SimulationManager::loadSimulationSetup(const QString& path, bool store)
{
    QFileInfo info(path);
    ...
    {
        LOG_ERROR() << "Path " << path << " does not exist.";
        return std::shared_ptr<SimulationSetup>();
    }
    ...
    std::shared_ptr<SimulationSetup> loadedSetup = loadSimulationSetup(setupDocument, path, fname);
...
std::shared_ptr<SimulationSetup> SimulationManager::saveSimulationSetup(std::shared_ptr<SimulationSetup> setup, bool overwrite, bool rememberPath)
{
    std::shared_ptr<SimulationSetup> returning = std::shared_ptr<SimulationSetup>();
    int err;
...
const std::shared_ptr<Simulation> SimulationManager::getSimulation() const
{
    return mSimulation;
}

std::shared_ptr<Simulation> SimulationManager::getSimulation()
{
    return mSimulation;
}
...
bool SimulationManager::initializeSimulationFromFile(const QString& path, bool reinitialize, bool loadData)
{
    ...
    if (!loaded)
    {
        std::shared_ptr<SimulationSetup> loadedSetup = loadSimulationSetup(file.absoluteFilePath(), true);
...
bool SimulationManager::initializeSimulation(const QString& name, bool reinitialize)
{
    ...
    std::shared_ptr<SimulationSetup> setup(mSetupList.at(getSetupIndex(name)));
...
std::shared_ptr<TaskDefinition> SimulationManager::newTaskDefinition(TaskDefinition::ETaskType type) const
{
    ...
    default:
        ...
        return std::shared_ptr<TaskDefinition>(new TaskDefinition());
        break;
    }
    ...
    return std::shared_ptr<TaskDefinition>(new TaskDefinition(type,
...
std::shared_ptr<SimulationSetup> SimulationManager::saveSimulationSetupByDialog(bool overwrite, bool saveData)
{
    if (!simulationInitialized())
    {
    ...
    std::shared_ptr<SimulationSetup> setup(mSimulation->getSetup());
...
std::shared_ptr<SimulationSetup> SimulationManager::loadSimulationSetup(QDomDocument& document, const QString& path, const QString& fname)
{
    //Parse document
    ...
    //Create the setup
    std::shared_ptr<SimulationSetup> target(
    ...
    while(!n.isNull())
    {
        ...
        else
        {
            ...
            if(!e.isNull() && (isPlugin || isFrame || isSignalPlugin))
            {
                ...
                //Add definition
                target.push_back(std::shared_ptr<PluginDefinition>(new PluginDefinition(nameValue, captionValue, pluginType, reloadDefinition, parameterValue, attachmentValue)));
   ...
   //Parse document
    ...
    while(!n.isNull())
    {
        ...
        else
        {
            ...
            //Create Definition
            target.push_back(std::shared_ptr<TaskDefinition>(new TaskDefinition(type, name, priority, first, second, third, firstList, secondList)));
...
std::shared_ptr<SimulationSetup> SimulationManager::saveSimulationSetupProtected(SimulationSetup& setup, int& err, bool directOverwrite, bool rememberPath)
{
    std::shared_ptr<SimulationSetup> ptr(new SimulationSetup(setup));
    return saveSimulationSetupProtected(ptr, err, directOverwrite, rememberPath);
}

std::shared_ptr<SimulationSetup> SimulationManager::saveSimulationSetupProtected(std::shared_ptr<SimulationSetup> setup, int& err, bool directOverwrite, bool rememberPath)
{
    ...
    if (!info.exists())
    {
        ...
        if (!file.open(QIODevice::WriteOnly))
        {
            ...
            file.close();
            err = 1;
            std::shared_ptr<SimulationSetup>();
        }
    ...
    std::shared_ptr<SimulationSetup> saveSetup(setup);
...
bool SimulationManager::saveTaskDefinitions(QDomDocument& doc, QDomElement& element, const SimulationSetup::TTaskDefinitions& source, const QString& msg)
{
    ...
    {
        std::shared_ptr<TaskDefinition> def(*it);
