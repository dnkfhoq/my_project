...
PropertyManager::PropertyManager(PropertyList* parent, std::shared_ptr<SparkController> controller) :
...
void PropertyManager::genProperties(TLeafPtr leaf, TClassList& classList, TProperties& propList) const
{
    ...
    for (auto class_it = classList.rbegin(); class_it != classList.rend(); ++class_it)
    {
        ...
        std::shared_ptr<PropertyControl> supporter(getSupportingControl(*class_it));
...
bool PropertyManager::writeProperty(std::shared_ptr<Property>& prop)
{
    std::shared_ptr<zeitgeist::Leaf> leaf(prop->mLeaf.lock());
    if (leaf.get() == 0)
    {
    ...
    std::shared_ptr<PropertyControl> supporter(getSupportingControl(prop->mClass));
...
bool PropertyManager::readProperty(std::shared_ptr<Property>& prop)
{
    std::shared_ptr<zeitgeist::Leaf> leaf(prop->mLeaf.lock());
    if (leaf.get() == 0)
    {
...
std::shared_ptr<PropertyControl> PropertyManager::getSupportingControl(ClassDescriptor& descriptor) const
{
    ...
    return std::shared_ptr<PropertyControl>();
}
...
void PropertyManager::getClassList(TLeafPtr leaf, TClassList& classList) const
{
    ...
    std::shared_ptr<SimSpark> spark = mParent->getSpark();
    if (spark.get() == 0)
        return;

    std::shared_ptr<zeitgeist::Class> classPtr = leaf->GetClass();
    if (classPtr.get() == 0)
        return;
...
void PropertyManager::registerPropertyControl(std::shared_ptr<PropertyControl> control)
{
    bool found = false;
...
void PropertyManager::unregisterPropertyControl(std::shared_ptr<PropertyControl> simControl)
{
...
void PropertyManager::getClassListPrivate(TClassPtr classPtr, TClassList& classList, const std::shared_ptr<SimSpark>& spark) const
{
    ...
    for (auto base_it = baseClasses.begin(); base_it != baseClasses.end(); base_it++)
    {
        ...
        std::shared_ptr<zeitgeist::Class> base = std::dynamic_pointer_cast<Class>
