...
SetupFrame::SetupFrame() :
    mSettings(GuiController::controller->getSettings())
{
    //UI Setup
    ui.setupUi(this);
    loadFrameDesign();
    ...
    //Default values
    GuiController* controller = GuiController::controller;
    mSimulationManager = controller->getSimulationManager();
    mSetupChanged = false;
    ...
    //GuiController connections. Important to connect to the controllers shutdown signal. Otherwise the managers might be destroyed during this frames destructor 
    connect(controller, SIGNAL(aboutToShutdown()), this, SLOT(cleanup()));

    //------------------------
    //Class-Information
    setClassInfo(PI_ICON_NAME, ":book");
}
...
/*TaskDefinition* SetupFrame::getCurrentServer()
{
    //Create definition for server and add to setup list
    if (!mCurrentSetup->hasTaskDefinition(TaskDefinition::TT_SERVERTHREAD))
        mCurrentSetup->insertTask(*mSimulationManager->newTaskDefinition(TaskDefinition::TT_SERVERTHREAD));

    //Get index of server definition
    int serverIndex = mCurrentSetup->findNextTaskDefinition(0, TaskDefinition::TT_SERVERTHREAD);

    //Return pointer to server definition
    return &*mCurrentSetup->getTaskDefinitions().at(serverIndex);;
}*/
...
void SetupFrame::showLoadTaskContextMenu(QPoint pos)
{
    ...
    QAction* action;
    clearActionReceivers();
    PluginManager* manager = GuiController::controller->getPluginManager();
...
int SetupFrame::insertSetup(shared_ptr<SimulationSetup> setup, bool copy)
{
...
void SetupFrame::removeSetup()
{
     if (checkSaved())
     {
         ...
         if (currentIndex < (mSetupList.size() - 1))
             nextIndex = currentIndex -1;
         else if (currentIndex < nextIndex)
             nextIndex = nextIndex - 1;

         //Remove from simulation manager (if contained)
...
void SetupFrame::renameSetup()
{
    if (checkSaved())
    {
       ...
       bool updated = mSimulationManager->updateSimulationSetup(*mCurrentSetup, true);
...
void SetupFrame::changeSetup(int index)
{
    ...
    int setupindex = mSimulationManager->getSetupIndexByFilePath(mSetupList.at(index)->getFileName());
    if (setupindex != -1)
    {
        mCurrentSetup = shared_ptr<SimulationSetup>(new SimulationSetup(*mSimulationManager->getSetupList().at(setupindex)));
        mLastSetupIndex = setupindex;
    }
...
/*void SetupFrame::updateServerDisplay()
{
    mReactToChangeSetup = false;

    if (mCurrentSetup->hasTaskDefinition(TaskDefinition::TT_SERVERTHREAD))
    {
        TaskDefinition* server = getCurrentServer();

        //Text
        ui.mainListFirstEdit->setText(server->getFirst());
        ui.mainListSecondEdit->setText(server->getSecond());
        ui.mainListThirdEdit->setText(server->getThird());

        //ListEntrys
        mChosenListEntryIllegal = false;
        mChosenListEntry = 0;
        ui.additionalListList->clear();
        for (auto it = server->getFirstList().begin(); it != server->getFirstList().end(); it++)
        {
            ui.additionalListList->addItem(*it);
            ui.additionalListList->setCurrentRow(ui.additionalListList->count() - 1);
            ui.additionalListList->currentItem()->setFlags(Qt::ItemIsEditable | Qt::ItemIsSelectable | Qt::ItemIsEnabled);

            //ListEntry color
            checkCurrentListEntry();
        }

        //Parameter
        ui.mainListParametersTextEdit->clear();
        for (auto it = server->getSecondList().begin(); it != server->getSecondList().end(); it++)
        {
            ui.mainListParametersTextEdit->insertPlainText(*it);
            if (it != server->getSecondList().end())
                ui.mainListParametersTextEdit->insertPlainText("\n");
        }
    }
    else
    {
        //Has no server
        ui.mainListFirstEdit->setText("");
        ui.mainListSecondEdit->setText("");
        ui.mainListThirdEdit->setText("");
        ui.additionalListList->clear();
        ui.mainListParametersTextEdit->clear();
        mChosenListEntryIllegal = false;
        mChosenListEntry = 0;
    ...
    firstCheckText(ui.mainListFirstEdit->text());
    secondCheckText(ui.mainListSecondEdit->text());
    thirdCheckText(ui.mainListThirdEdit->text());
}
*/
...
{
    if (mCurrentSetup->getFileName().compare(path) == 0)
    {
        //Update filename
        mCurrentSetup->setFileName(mSimulationManager->getSetupList().at(mSimulationManager->getSetupIndex(setup))->getFileName());
    }
    updateSetupListDisplay();
}
...
void SetupFrame::updateSetupListDisplay()
{
    ...
    //Store chosen setup
    shared_ptr<SimulationSetup> chosenSetup;
    if (mSetupList.size() != 0)
    ...
    for (auto it = mSetupList.begin(); it != mSetupList.end(); it++)
    {
        if (chosenSetup.get()) //has chosen setup
        if (it->get()->getFileName().compare(chosenSetup->getFileName()) == 0) //and file is identical
            ui.setupComboBox->setCurrentIndex(pos);

        //Unsaved?
        if (mSetupChanged)
        {
            QString val = QString("%1 (unsaved)").arg(mSimulationManager->getSetupList().at(pos)->getName());
...
/*
void SetupFrame::loadFirst()
{
   TaskDefinition* def = getCurrentServer();
   if (def == 0)
       return;
  
   //Load executable from file dialog and fill in line edit
   loadTaskEntry(0, ui.mainListFirstEdit);
}

void SetupFrame::loadSecond()
{
    TaskDefinition* def = getCurrentServer();
    if (def == 0)
        return;
    ...
    loadTaskEntry(1, ui.mainListSecondEdit);
}

void SetupFrame::loadThird()
{
    TaskDefinition* def = getCurrentServer();
    if (def == 0)
        return;
    ...
    loadTaskEntry(2, ui.mainListThirdEdit);
}

void SetupFrame::firstCheckText(QString text)
{
    if (!mReactToChangeSetup)
        return;
        ...
        mSimulationManager->hasExecutable(text))
    {
        ...
        QPalette pal = ui.mainListFirstEdit->palette();
        pal.setColor(QPalette::Text, Qt::black);
        ui.mainListFirstEdit->setPalette(pal);
    }
    else
    {
...
void SetupFrame::firstChangeText()
{
    if (!mReactToChangeSetup)
        return;
    ...
    QString text = ui.mainListFirstEdit->text();
...
void SetupFrame::secondCheckText(QString text)
{
    if (!mReactToChangeSetup)
        return;
    ...
    {
        //ListEntry found, black text
        QPalette pal = ui.mainListSecondEdit->palette();
        pal.setColor(QPalette::Text, Qt::black);
        ui.mainListSecondEdit->setPalette(pal);
    }
    else
    {
        //ListEntry not found, red text
...
void SetupFrame::secondChangeText()
{
    if (!mReactToChangeSetup)
        return;
    ...
    QString text = ui.mainListSecondEdit->text();
...
void SetupFrame::thirdCheckText(QString text)
{
    if (!mReactToChangeSetup)
        return;
    ...
    {
        //ListEntry found, black text
        QPalette pal = ui.mainListThirdEdit->palette();
        pal.setColor(QPalette::Text, Qt::black);
        ui.mainListThirdEdit->setPalette(pal);
    }
    else
    {
        //ListEntry not found, red text
...
void SetupFrame::thirdChangeText()
{
    if (!mReactToChangeSetup)
        return;
    ...
    QString text = ui.mainListThirdEdit->text();
...
void SetupFrame::editParameters()
{
    if (!mReactToChangeSetup)
        return;
    ...
    //Add parameters from text edit
    QString parameter = ui.mainListParametersTextEdit->toPlainText();

    def->clearSecondList();
    
    int start = 0;
    int end = parameter.indexOf('\n');
    if (end == -1) end = parameter.size();
    while (start < parameter.size())
    {
        def->addToSecondList(parameter.mid(start, end-start));
        start = end+1;
        end = parameter.indexOf('\n', start);
        if (end == -1) end = parameter.size();
    }
...
void SetupFrame::addListEntry()
{
    ...
    def->addToFirstList("entry");

    ui.additionalListList->addItem("entry");
    ui.additionalListList->item(ui.additionalListList->count()-1)->setFlags(Qt::ItemIsEditable | Qt::ItemIsSelectable | Qt::ItemIsEnabled);
    ui.additionalListList->setCurrentRow(ui.additionalListList->count()-1);
...
void SetupFrame::removeListEntry()
{
    if (!mReactToChangeSetup)
        return;

    if (ui.additionalListList->count() == 0)
        return;
    ...
    if (mChosenListEntry < 0 || mChosenListEntry >= ui.additionalListList->count())
    {
        LOG_ERROR() << "Index out of bounds.";
        return;
    }

    if (def->getFirstList().size() != ui.additionalListList->count())
    {
    ...
    //Erase
    mChosenListEntryIllegal = false;
    int thisListEntry = mChosenListEntry;
    def->removeFromFirstList(thisListEntry);
    ui.additionalListList->takeItem(thisListEntry);
    
    //Select next
    if (ui.additionalListList->count() > 0)
    {
        int nextListEntry = thisListEntry < ui.additionalListList->count() ? thisListEntry : thisListEntry-1;
        ui.additionalListList->setCurrentRow(nextListEntry);
        mChosenListEntry = nextListEntry;
    }
    else
        mChosenListEntry = -1;
...
void SetupFrame::moveListEntryUp()
{
    if (!mReactToChangeSetup)
        return;
    ...
    //Switch
    bool currentIllegal = mChosenListEntryIllegal;
    mChosenListEntryIllegal = false;
    int thisListEntry = mChosenListEntry;
    QString thisListEntryText = ui.additionalListList->item(thisListEntry)->text();
    def->swapInFirstList(thisListEntry, thisListEntry-1);
    ui.additionalListList->takeItem(thisListEntry);
    ui.additionalListList->insertItem(thisListEntry-1, thisListEntryText);
    ui.additionalListList->item(thisListEntry-1)->setFlags(Qt::ItemIsEditable | Qt::ItemIsSelectable | Qt::ItemIsEnabled);
    mChosenListEntryIllegal = currentIllegal;
    
    //Select moved one
    ui.additionalListList->setCurrentRow(thisListEntry-1);
    mChosenListEntry = thisListEntry-1;
    checkCurrentListEntry();
...
void SetupFrame::moveListEntryDown()
{
    if (!mReactToChangeSetup)
        return;
    ...
    if (mChosenListEntry == ui.additionalListList->count() - 1)
        return;
...
void SetupFrame::checkListEntry(QListWidgetItem* item)
{
    if (!mReactToChangeSetup)
        return;
...
void SetupFrame::checkCurrentListEntry()
{
    if (!mReactToChangeSetup)
        return;

    checkListEntry(ui.additionalListList->currentItem());
}

void SetupFrame::editListEntry(QListWidgetItem* item)
{
    if (!mReactToChangeSetup || !mReactToEditListEntry)
        return;
    ...
    //Set color
    mReactToEditListEntry = false;
    checkListEntry(item);

    if (ui.additionalListList->currentItem() == item)
    if ( item->textColor() == QColor(Qt::red))
    {
        //Save it, but send a warning before changing
        mChosenListEntryIllegal = true;
    }
    else
    {
        mChosenListEntryIllegal = false;
...
void SetupFrame::editCurrentListEntry(QString text)
{
    if (!mReactToChangeSetup)
        return;

    editListEntry(ui.additionalListList->currentItem());
}

void SetupFrame::chooseListEntry(int row)
{
    if (mChosenListEntryIllegal)
    {
        if (ui.additionalListList->count() > mChosenListEntry && mChosenListEntry >= 0)
            LOG_INFO() << "ListEntry " << ui.additionalListList->item(mChosenListEntry)->text() << " not found. Add its path or the single file to the SimulationManagers script include paths.";
    }

    mChosenListEntry = row;
    mChosenListEntryIllegal = false;
}
*/
...
void SetupFrame::chooseLoadPlugin(int index)
{
    ...
    if (mCurrentSetup->getAddPlugins().size() <= index)
    {
        LOG_ERROR() << "Index out of bounds.";
        return;
    }
...
void SetupFrame::chooseUnloadPlugin(int index)
{
    ...
    if (mCurrentSetup->getRemovePlugins().size() <= index)
    {
        LOG_ERROR() << "Index out of bounds.";
        return;
    }
...
void SetupFrame::chooseTask(int index)
{
    ...
    if (mCurrentSetup->getTaskDefinitions().size() <= index)
    {
        LOG_ERROR() << "Index out of bounds.";
        return;
    }
...
/*
void SetupFrame::showExplorerTreeViewContextMenu(QPoint pos)
{
    showExplorerTreeViewContextMenu(pos, ui.explorerTreeView, mExplorerItemIndex, true);
}

void SetupFrame::onListEntryPathClick(int number)
{
    scrollToRegisteredExplorerFile(number, true, false);
}

void SetupFrame::onFirstPathClick(int number)
{
    scrollToRegisteredExplorerFile(number, true, true);
}

void SetupFrame::setFirstTextFromExplorer(bool absolute)
{
    setLineEditTextFromExplorer(ui.explorerTreeView, mExplorerItemIndex, ui.mainListFirstEdit, true, true, absolute);
    firstChangeText();
}

void SetupFrame::setSecondTextFromExplorer(bool absolute)
{
    setLineEditTextFromExplorer(ui.explorerTreeView, mExplorerItemIndex, ui.mainListSecondEdit, true, false, absolute);
    secondChangeText();
}

void SetupFrame::setThirdTextFromExplorer(bool absolute)
{
    setLineEditTextFromExplorer(ui.explorerTreeView, mExplorerItemIndex, ui.mainListThirdEdit, true, false, absolute);
    thirdChangeText();
}*/
