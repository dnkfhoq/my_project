...
{
    mFileName = "";
    mData = std::shared_ptr<Settings>();
    mSetup = std::shared_ptr<SimulationSetup>(new SimulationSetup());
...
const std::shared_ptr<SimulationSetup>& Simulation::getSetup() const    
{
    return mSetup;
}

std::shared_ptr<SimulationSetup>& Simulation::getSetupNc()
{
    return mSetup;
}
...
std::shared_ptr<Settings> Simulation::getData() const
{
    return mData;
}
...
std::shared_ptr<SimulationTask> Simulation::getTask(int index) const
{
...
bool Simulation::initialize(const SimulationSetup& setup)
{
    ...
    mData = std::shared_ptr<Settings>(new Settings(mFileName, 0, QSettings::IniFormat)); //Closing old data will save them automatically. 
...
bool Simulation::stop(bool stopserver)
{
    ...
    for (auto it = mTasks.begin(); it != mTasks.end(); it++)
    {
        ...
        /*if (task->getTaskDefinition().getType() == TaskDefinition::TT_SERVERTHREAD && stopserver == false)
        {
            ...
            skipServer = true;
            std::dynamic_pointer_cast<SparkSimulationThread>(*it)->reset();
        }
        else*/
        {
            //Unpause
    ...
    for (int run = 1; run < 401; run++)
    {
        //Count closing tasks
        int not_closed = 0;
        std::shared_ptr<SimulationTask> first = std::shared_ptr<SimulationTask>();
        for (auto it = mTasks.begin(); it != mTasks.end(); it++)
        {
            SimulationTask* task = it->get();
            ...
            {
                not_closed++;
                first = std::shared_ptr<SimulationTask>(*it);
            }
        }
...
bool Simulation::stopTask(SimulationTask* task)
{
    ...
    std::shared_ptr<ServerThread> server(Carbon::get()->getSimulationManager()->getSparkServerThread());
...
std::shared_ptr<TaskDefinition> Simulation::addNewTask(TaskDefinition& taskDefinition)
{
    if (!isInitialized())
    {
        LOG_ERROR() << "Cannot add new task before initialization of the Simulation.";
        return std::shared_ptr<TaskDefinition>();
    }

    //Inserting copies the definition
    std::shared_ptr<TaskDefinition> newDefinition(mSetup->insertTask(taskDefinition));
...
bool Simulation::addTask(std::shared_ptr<TaskDefinition> taskDefinition)
{
    std::shared_ptr<SimulationTask> task = std::shared_ptr<SimulationTask>();
    SimulationManager* manager = Carbon::get()->getSimulationManager();
    std::shared_ptr<ServerThread> server = manager->getSparkServerThread();
    ...
    if (taskDefinition->getType() == TaskDefinition::TT_SERVERTHREAD)
    {
        ...
        task = std::shared_ptr<SimulationTask>(std::dynamic_pointer_cast<SimulationTask>(server));
    }
    else
    {
        //Non-server tasks
        ...
        //Success
        task = std::shared_ptr<SimulationTask>(ptr);
    }
    ...
    if (taskDefinition->getType() == TaskDefinition::TT_SERVERTHREAD)
    {
        emit serverCreated(std::static_pointer_cast<ServerThread>(task));
    }
...
void Simulation::startTask(std::shared_ptr<SimulationTask> task, bool restart)
{
