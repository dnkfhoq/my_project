...
Property::Property(std::shared_ptr<zeitgeist::Leaf> leaf, ClassDescriptor descriptor, const QString& name, const QString& caption,
...
const std::shared_ptr<const Data> Property::getCurrentValue() const
{
...
const std::shared_ptr<const Data> Property::getValue() const
{
    return mValue;
}

const std::shared_ptr<const Data> Property::getTempValue() const
{
    return mTempValue;
}
...
{
    return setValue(std::shared_ptr<Data>(data));
}

bool Property::setValue(std::shared_ptr<Data> data)
{
    bool ok = false;
    if (mValueSemaphore.tryAcquire(1, 100))
    {
        ...
        if (ok)
        {
            mTempValue = std::shared_ptr<Data>(data);
        }
...
bool Property::setValue(const QString& dataString)
{
    ...
    if (mValueSemaphore.tryAcquire(1, 100))
    {
        ...
        if (ok)
        {
            std::shared_ptr<Data> data(Data::createData(getDataType(), dataString));
            if (data.get() == 0)
            {
...
std::weak_ptr<zeitgeist::Leaf> Property::getLeaf()
{
    return mLeaf;
}
...
{
    setValueProtected(std::shared_ptr<Data>(data));
}

// Protected write does not use semaphore, because it was already acquired by the PropertyExecuter
void Property::setValueProtected(std::shared_ptr<Data> data)
{
    bool ok = true;
