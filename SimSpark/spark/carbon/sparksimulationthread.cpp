...
SparkSimulationThread::SparkSimulationThread(std::shared_ptr<TaskDefinition> definition) :
...
//--------------------------------------------------------------
/*
std::shared_ptr<SparkSimulationThread> SparkSimulationThread::getHostSparkThread()
{
    if (mSpark.get() != 0)
    {
        ...
        if (index == -1)
            ...
            return std::static_pointer_cast<SparkSimulationThread>(manager->getThreadList().at(index));
    }
    else
    {
    ...
    return std::shared_ptr<SparkSimulationThread>();
}*/

std::shared_ptr<SparkController> SparkSimulationThread::getSparkController() const
{
    return mController;
}
