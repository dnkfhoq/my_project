...
bool SimSpark::Init(int argc, char** argv)
{
    ...
    // init library cores
    mZeitgeist = std::shared_ptr<zeitgeist::Zeitgeist>(new zeitgeist::Zeitgeist("." PACKAGE_NAME, mRelPathPrefix));
    mOxygen = std::shared_ptr<oxygen::Oxygen>(new oxygen::Oxygen(*mZeitgeist));
    mKerosin = std::shared_ptr<kerosin::Kerosin>(new kerosin::Kerosin(*mZeitgeist));
...
bool SimSpark::Reset()
{
    ...
    std::shared_ptr<zeitgeist::Leaf> leaf(GetCore()->Get(pathStr));
    std::shared_ptr<oxygen::Scene> scene = std::shared_ptr<oxygen::Scene>(std::static_pointer_cast<oxygen::Scene>(leaf));
    std::shared_ptr<oxygen::World> world;
    std::shared_ptr<oxygen::Space> space;

    for (auto it = scene->begin(); it != scene->end(); it++)
    {
        if (it->get()->GetName().compare("world") == 0)
        {
            world = std::static_pointer_cast<oxygen::World>(*it);
    ...
    for (auto it = scene->begin(); it != scene->end(); it++)
    {
        ...
        {
             space = std::static_pointer_cast<oxygen::Space>(*it);
             scene->RemoveChildReference(*it);
             break;
        }
    }

    //std::shared_ptr<oxygen::GameControlServer> gc = std::static_pointer_cast<oxygen::GameControlServer>(GetCore()->Get("/sys/server/gamecontrol/"));
    //std::shared_ptr<zeitgeist::Leaf> gc_state = gc->GetChild("GameStateAspect");
    //std::shared_ptr<zeitgeist::Leaf> gc_ball = gc->GetChild("BallStateAspect");
    //std::shared_ptr<zeitgeist::Leaf> gc_rule = gc->GetChild("SoccerRuleAspect");
    //
    //gc->UnlinkChildren();
    //gc->Unlink();
    //gc_state.reset();
    ...
    std::shared_ptr<zeitgeist::Node> root = std::static_pointer_cast<zeitgeist::Node>(GetCore()->GetRoot());
    std::shared_ptr<zeitgeist::Leaf> root_leaf(GetCore()->GetRoot());
...
void SimSpark::AddLogStream(std::shared_ptr<LogStream> stream, zeitgeist::LogServer::EPriorityLevel level, bool syncStream)
{
    if (mState == SSS_PREINIT)
...
int SimSpark::ExecuteRunScripts()
{
    ...
    std::shared_ptr<zeitgeist::ScriptServer> scriptServer = GetCore()->GetScriptServer();
