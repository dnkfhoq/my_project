...
bool EvalScript::execute()
{
    ...
    std::shared_ptr<ScriptServer> server = std::shared_ptr<ScriptServer>(getController()->getSpark()->GetScriptServer());
    if (!server.get())
        server = std::shared_ptr<ScriptServer>(getController()->getSpark()->GetCore()->GetScriptServer());
...
bool ImportScene::execute()
{
    ...
    std::shared_ptr<oxygen::SceneImporter> sceneImporter = 
        std::static_pointer_cast<oxygen::SceneImporter>(getController()->getSpark()->GetCore()->Get("sys/server/scene/RubySceneImporter"));
