...
std::shared_ptr<PluginDefinition> SimulationSetup::insertAddPlugin(const PluginDefinition& addplugin)
{
    //Only plugin definitions that specify a plugin completely can be used as "addPlugin" definition
    std::shared_ptr<PluginDefinition> plugin(new PluginDefinition(addplugin));
    bool success = insertAddPlugin(plugin);

    if (success)
        return plugin;

    return std::shared_ptr<PluginDefinition>();
}

bool SimulationSetup::insertAddPlugin(std::shared_ptr<PluginDefinition> addplugin)
{
    //Only plugin definitions that specify a plugin completely can be used as "addPlugin" definition
...
std::shared_ptr<PluginDefinition> SimulationSetup::insertRemovePlugin(const PluginDefinition& removeplugin)
{
    ...
    std::shared_ptr<PluginDefinition> plugin(new PluginDefinition(removeplugin));
...
bool SimulationSetup::insertRemovePlugin(std::shared_ptr<PluginDefinition> removeplugin)
{
    //Vague plugin definitions that specify a range of plugins can be used as "removePlugin" definition
    //Detailed data (like task definitions) are not needed
...
std::shared_ptr<TaskDefinition> SimulationSetup::insertTask(const TaskDefinition& task)
{
    mTaskDefinitions.push_back(std::shared_ptr<TaskDefinition>(new TaskDefinition(task)));
    mSaved = false;
...
void SimulationSetup::readData(const SimulationSetup& source, bool deep)
{
    ...
    if (deep)
    {
        ...
        for (auto it = source.mTaskDefinitions.begin(); it != source.mTaskDefinitions.end(); it++)
            mTaskDefinitions.push_back(std::shared_ptr<TaskDefinition>(new TaskDefinition(**it)));
        for (auto it = source.mAddPlugins.begin(); it != source.mAddPlugins.end(); it++)
            mAddPlugins.push_back(std::shared_ptr<PluginDefinition>(new PluginDefinition(**it)));
        for (auto it = source.mRemovePlugins.begin(); it != source.mRemovePlugins.end(); it++)
            mRemovePlugins.push_back(std::shared_ptr<PluginDefinition>(new PluginDefinition(**it)));
    }
    else
    {
        mTaskDefinitions = source.mTaskDefinitions;
