...
std::shared_ptr<Carbon::CarbonLogControl> Carbon::mCarbonLogControlStatic = 
    std::shared_ptr<Carbon::CarbonLogControl>(new Carbon::CarbonLogControl());
    ...
    mClosed(false)
{
    ...
    mCarbonLogControl = std::shared_ptr<CarbonLogControl>(Carbon::mCarbonLogControlStatic);
...
std::shared_ptr<Settings> Carbon::getSettings() const
{
    return mSettings;
}
...
bool Carbon::initGui()
{
    ...
    qRegisterMetaType<std::shared_ptr<LogMessage>>("LogMessageSPtr");
    qRegisterMetaType<std::shared_ptr<SparkProperty::Property>>("SparkPropertySPtr");
    qRegisterMetaType<std::shared_ptr<Topic>>("TopicSPtr");
...
bool Carbon::loadLayout()
{
    ...
    //Search file
    std::shared_ptr<QLibrary> lib = loader.getLibraryByFileName(mLayoutFile);
    if (!lib.get())
    {
...
bool Carbon::initLogging()
{
    ...
    std::shared_ptr<CarbonLogControl> control(getLogControl());
...
std::shared_ptr<Carbon::CarbonLogControl> Carbon::getLogControl()
{
    if (Carbon::get() != 0)
