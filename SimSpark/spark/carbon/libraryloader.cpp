...
bool LibraryLoader::load(const std::shared_ptr<QLibrary>& search)
{
...
bool LibraryLoader::unload(const std::shared_ptr<QLibrary>& search)
{
    LibraryMap::Iterator lib(findLibraryDefinition(search));
    return (lib == mLibraries.end() ? false : unload(*lib));
}
...
bool LibraryLoader::isLoaded(const std::shared_ptr<QLibrary>& search) const
{
...
bool LibraryLoader::exportPlugins(const std::shared_ptr<QLibrary>& search)
{
    LibraryMap::Iterator lib(findLibraryDefinition(search));
    return (lib == mLibraries.end() ? false : exportPlugins(*lib));
}
...
QMainWindow* LibraryLoader::exportLayout(const std::shared_ptr<QLibrary>& search)
{
    LibraryMap::Iterator lib(findLibraryDefinition(search));
    return (lib == mLibraries.end() ? 0 : exportLayout(*lib));
}

const std::shared_ptr<QLibrary> LibraryLoader::getLibrary(const QString& path) const
{
    return getLibraryNc(path);
}

const std::shared_ptr<QLibrary> LibraryLoader::getLibrary(int id) const
{
    return getLibraryNc(id);
}

const std::shared_ptr<QLibrary> LibraryLoader::getLibraryByFileName(const QString& path) const
{
    auto pos = findLibraryDefinitionByFileName(path);
    if (pos == mLibraries.end())
        return std::shared_ptr<QLibrary>();
    return pos->mLibrary;
}

QString LibraryLoader::getErrorString(const QString& path) const
{
    std::shared_ptr<QLibrary> lib(getLibraryNc(path));
    if (lib == 0)
...
{
    std::shared_ptr<QLibrary> lib(getLibraryNc(id));
    if (lib == 0)
...
int LibraryLoader::addLibrary(const QString& path)
{
    ...
    if (pos == mLibraries.end())
    {
        ...
        pos = mLibraries.insert(path, LibraryDefinition(getNextId(), std::shared_ptr<QLibrary>(lib)));
    }
...
std::shared_ptr<QLibrary> LibraryLoader::getLibraryNc(const QString& path) const
{
    auto pos = mLibraries.find(path);
    if (pos == mLibraries.end())
        return std::shared_ptr<QLibrary>();
...
std::shared_ptr<QLibrary> LibraryLoader::getLibraryNc(int id) const
{
    for (auto it = mLibraries.begin(); it != mLibraries.end(); it++)
    {
        if (it->mId == id)
...
LibraryLoader::LibraryMap::ConstIterator LibraryLoader::findLibraryDefinition(const std::shared_ptr<QLibrary>& lib) const
{
    for (auto it = mLibraries.begin(); it != mLibraries.end(); it++)
    {
...
LibraryLoader::LibraryMap::Iterator LibraryLoader::findLibraryDefinition(const std::shared_ptr<QLibrary>& lib)
{
    for (auto it = mLibraries.begin(); it != mLibraries.end(); it++)
    {
        if (it->mLibrary.get() == lib.get())
