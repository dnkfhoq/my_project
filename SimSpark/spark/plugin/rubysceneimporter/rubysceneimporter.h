...
protected:
    ...
    {
        std::weak_ptr<zeitgeist::Node> node;
        std::string method;
    ...
    struct ParamEnv
    {
        ...
        ParamEnv() {};
        ParamEnv(std::shared_ptr<zeitgeist::ParameterList> p)
            : parameter(p) {};
    };
...
protected:
    ...
    bool ReadGraph(sexp_t* sexp, std::shared_ptr<oxygen::BaseNode> root);
    bool ReadDeltaGraph(sexp_t* sexp, std::shared_ptr<oxygen::BaseNode> root);
    std::shared_ptr<zeitgeist::Object> CreateInstance(const std::string& className);

    void PushInvocation(const MethodInvocation& invoc);
    bool Invoke(const MethodInvocation& invoc);
    bool InvokeMethods();
    bool ReadMethodCall(sexp_t* _sexp, std::shared_ptr<oxygen::BaseNode> node);

    bool ReplaceVariable(std::string& param);
    bool EvalParameter(sexp_t* sexp, std::string& value);

    std::shared_ptr<oxygen::BaseNode> CreateNode(sexp_t* sexp);
    bool ParseTemplate(sexp_t* sexp);
    bool ParseDefine(sexp_t* sexp);
    // bool ParseSwitch(sexp_t* sexp, std::string& value);
    bool ParseSwitch(sexp_t* sexp, std::shared_ptr<oxygen::BaseNode> root);

    void PushParameter(std::shared_ptr<zeitgeist::ParameterList> parameter);
    void PopParameter();
