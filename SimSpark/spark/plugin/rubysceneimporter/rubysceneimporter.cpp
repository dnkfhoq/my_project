...
bool RubySceneImporter::ParseScene(const char* scene, int size,
                                   std::shared_ptr<oxygen::BaseNode> root,
...
void RubySceneImporter::PushParameter(std::shared_ptr<ParameterList> parameter)
{
...
std::shared_ptr<Object> RubySceneImporter::CreateInstance(const string& className)
{
...
RubySceneImporter::CreateNode(sexp_t* sexp)
{
    ...
    std::shared_ptr<Object> obj = CreateInstance(className);
    ...
    std::shared_ptr<BaseNode> node = std::dynamic_pointer_cast<BaseNode>(obj);
...
bool RubySceneImporter::EvalParameter(sexp_t* sexp, string& value)
{
    ...
    if (! script->Eval(expr, scriptvalue))
        {
            GetLog()->Error()
                << "(RubySceneImporter) ERROR: in file '"
                << mFileName
    ...
    if (! scriptvalue.GetString(value))
        {
            GetLog()->Error()
                << "(RubySceneImporter) ERROR: in file '"
                << mFileName
...
{
    std::shared_ptr<Class> baseNodeClass =
        std::dynamic_pointer_cast<Class>(GetCore()->Get("/classes/oxygen/Transform"));
...
bool RubySceneImporter::Invoke(const MethodInvocation& invoc)
{
    ...
    std::shared_ptr<Node> node = invoc.node.lock();
    std::shared_ptr<Class> theClass = node->GetClass();
...
bool RubySceneImporter::ReadMethodCall(sexp_t* sexp, std::shared_ptr<BaseNode> node)
{
    if (sexp == 0)
        {
            return false;
...
bool RubySceneImporter::ParseSwitch(sexp_t* sexp, std::shared_ptr<oxygen::BaseNode> root)
{
    if (sexp == 0)
    {
        return false;
...
bool
RubySceneImporter::ReadDeltaGraph(sexp_t* sexp, std::shared_ptr<BaseNode> root)
{
        ...
        case SEXP_LIST:
            {
                ...
                if (sub != 0)
                {
                        ...
                        )
                    {
                        if (iter != root->end())
                        {
                            node = std::dynamic_pointer_cast<BaseNode>(*iter);
                            ++iter;
                        }
                    } else {
                        node = root;
...
bool
RubySceneImporter::ReadGraph(sexp_t* sexp, std::shared_ptr<BaseNode> root)
{
    while (sexp != 0)
    {
        switch (sexp->ty)
        ...
        case SEXP_VALUE:
            {
                ...
                {
                    sexp = sexp->next;
                    std::shared_ptr<BaseNode> node = CreateNode(sexp);
                ...
                else if (name == S_SELECT)
                {
                    ...
                    std::shared_ptr<BaseNode> node =
                        std::dynamic_pointer_cast<BaseNode>(root->GetChild(name));
