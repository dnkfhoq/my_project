...
bool
PerfectVisionPerceptor::Percept(std::shared_ptr<PredicateList> predList)
{
    if (mSceneServer.get() == 0)
    {
        mSceneServer = std::static_pointer_cast<SceneServer>
    ...
    std::shared_ptr<Scene> activeScene = mSceneServer->GetActiveScene();
    ...
    {
        std::shared_ptr<Transform> j = std::static_pointer_cast<Transform>(*i);
