    ...
    for (TEntryMap::iterator i = mEntryMap.begin(); i != mEntryMap.end(); ++i)
        {
            ...
            if (!(cur->attr&16))
                {
                    if (std::regex_match(cur->filename, regExpression))
                        {
...
bool FileSystemZIP::SetPath(const string& inName)
{
    ...
    // get a stdio file
    mHandle = std::shared_ptr<StdFile>(new StdFile());
...
std::shared_ptr<salt::RFile> FileSystemZIP::Open(const string& inName)
{
    char            *fileName;
    TArchiveEntry   *cur;
    TLocalHeader    lh;
