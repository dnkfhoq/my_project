...
static int EventFilterCallback(void* /*userdata*/, SDL_Event *event)
{
    if (gInputSystem)
        {
...
bool InputSystemSDL::CreateDevice(const std::string &deviceName)
{
    ...
    std::shared_ptr<InputDevice> device =
        std::static_pointer_cast<InputDevice>(GetCore()->New(mangledName));
    ...
    if (mangledName.compare("TimerSDL") == 0)
        {
            mTimer = std::static_pointer_cast<TimerSDL>(device);
        }
    else
        {
            SDL_LockMutex(mMutex);
...
int InputSystemSDL::EventFilter(const SDL_Event *event)
{
    ...
    for (TLeafList::iterator i = mChildren.begin(); i!=mChildren.end(); ++i)
        {
            std::shared_ptr<InputDeviceSDL> device = std::static_pointer_cast<InputDeviceSDL>(*i);
