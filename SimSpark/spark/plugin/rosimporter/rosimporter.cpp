...
std::shared_ptr<float[]> RosImporter::TVertexList::GetPos()
{
    if (pos.get() != 0)
        {
            return pos;
    ...
    pos = std::shared_ptr<float[]> (new float[vertexRef.size() * 3]);
...
bool RosImporter::ImportScene(const string& fileName,
                              ...
                              std::shared_ptr<ParameterList> parameter)
{
    // try to open the file
    std::shared_ptr<salt::RFile> file = GetFile()->OpenResource(fileName);
...
void RosImporter::ApplyTransform(std::shared_ptr<Transform> transform, const Trans& trans)
{
    transform->SetLocalTransform(trans.matrix);
}

std::shared_ptr<Transform> RosImporter::CreateTransform(std::shared_ptr<BaseNode> parent, const Trans& trans)
{
    std::shared_ptr<Transform> transform = std::dynamic_pointer_cast<Transform>
...
std::shared_ptr<Transform> RosImporter::GetContextTransform(std::shared_ptr<BaseNode> parent, const Trans& trans)
{
    ...
    std::shared_ptr<Leaf> current(parent);
    while (current.get() != 0)
        {
            ...
            std::shared_ptr<Transform> transform = std::dynamic_pointer_cast<Transform>(current);
            if (transform.get() != 0)
                {
    ...
    for (
         ...
         )
        {
            std::shared_ptr<Transform> transform
                = std::static_pointer_cast<Transform>(*iter);
...
void RosImporter::SetJointBody(std::shared_ptr<RigidBody> body)
{
...
std::shared_ptr<RigidBody> RosImporter::GetContextBody(std::shared_ptr<BaseNode> parent)
{
    RosContext& context = GetContext();
    if (
        (! context.movable) ||
    ...
    context.body = std::dynamic_pointer_cast<RigidBody>
...
{
    std::shared_ptr<MaterialServer> materialServer = std::dynamic_pointer_cast<MaterialServer>
    ...
    std::shared_ptr<MaterialSolid> material = std::dynamic_pointer_cast<MaterialSolid>
...
bool RosImporter::ReadAmbientLight(TiXmlElement* element)
{
    ...
    std::shared_ptr<RenderServer> renderServer = std::dynamic_pointer_cast<RenderServer>
...
bool RosImporter::ReadScene(std::shared_ptr<BaseNode> parent, TiXmlElement* element)
{
...
bool RosImporter::ReadChildElements(std::shared_ptr<BaseNode> parent, TiXmlElement* element)
{
    for (
...
bool RosImporter::ReadCompound(std::shared_ptr<BaseNode> parent, TiXmlElement* element)
{
    string name;
    Trans trans;
    ...
    // transform
    std::shared_ptr<Transform> contextTransform = GetContextTransform(parent,trans);
...
bool RosImporter::ReadElements(std::shared_ptr<BaseNode> parent, TiXmlElement* element)
{
...
bool RosImporter::ReadMovable(std::shared_ptr<BaseNode> parent, TiXmlElement* element)
{
    RosContextScope scope(this);
...
std::shared_ptr<TransformCollider> RosImporter::CreateTransformCollider(std::shared_ptr<BaseNode> parent, const Trans& trans)
{
    std::shared_ptr<TransformCollider> transCollider = std::dynamic_pointer_cast<TransformCollider>
...
std::shared_ptr<ContactJointHandler> RosImporter::CreateContactJointHandler()
{
    std::shared_ptr<ContactJointHandler> handler = std::dynamic_pointer_cast<ContactJointHandler>
...
bool RosImporter::ReadCylinder(std::shared_ptr<BaseNode> parent, TiXmlElement* element)
{
    GetLog()->Debug() << "(RosImporter) cylinder geom unsupported yet. Created a capsule geom\n";
    return ReadCapsule(parent, element);
}

bool RosImporter::ReadCapsule(std::shared_ptr<BaseNode> parent, TiXmlElement* element)
{
    string name;
    double radius;
    double height;
    ...
    // transform
    std::shared_ptr<Transform> contextTransform = GetContextTransform(parent, Trans());
    std::shared_ptr<Transform> transform = CreateTransform(contextTransform, trans);
    transform->SetName(name);

    // visual
    std::shared_ptr<Capsule> capsule = std::dynamic_pointer_cast<Capsule>
    ...
    // physical
    std::shared_ptr<RigidBody> body = GetContextBody(transform);
    if (body.get() != 0)
        {
    ...
    if (physical.canCollide)
        {
            // geometry
            std::shared_ptr<CapsuleCollider> collider = std::dynamic_pointer_cast<CapsuleCollider>
            ...
            // collision handler
            std::shared_ptr<ContactJointHandler> handler = CreateContactJointHandler();
...
bool RosImporter::ReadSphere(std::shared_ptr<BaseNode> parent, TiXmlElement* element)
{
    string name;
    double radius;
    Trans trans;
    ...
    // visual
    std::shared_ptr<Sphere> sphere = std::dynamic_pointer_cast<Sphere>
    ...
    // geometry
    std::shared_ptr<SphereCollider> collider = std::dynamic_pointer_cast<SphereCollider>
...
bool RosImporter::ReadBox(std::shared_ptr<BaseNode> parent, TiXmlElement* element)
{
    string name;
    double length;
    double width;
    ...
    // visual
    std::shared_ptr<Box> box = std::dynamic_pointer_cast<Box>
    ...
    if (physical.canCollide)
        {
            // geometry
            std::shared_ptr<BoxCollider> collider = std::dynamic_pointer_cast<BoxCollider>
...
std::shared_ptr<RigidBody> RosImporter::GetJointParentBody()
{
    if (mStack.size() < 2)
        {
    ...
    for (;
         ...
         )
        {
            ...
            std::shared_ptr<RigidBody> body = context.body;
...
std::shared_ptr<RigidBody> RosImporter::GetJointChildBody(std::shared_ptr<BaseNode> parent)
{
    if (parent.get() == 0)
        {
            return std::shared_ptr<RigidBody>();
    ...
    for (Leaf::TLeafList::iterator iter = parent->begin();iter != parent->end();++iter)
        {
            std::shared_ptr<RigidBody> body = std::dynamic_pointer_cast<RigidBody>(*iter);
            if (body.get() == 0)
                {
                    continue;
                }
    ...
    for (Leaf::TLeafList::iterator iter = parent->begin();iter != parent->end();++iter)
        {
            std::shared_ptr<BaseNode> node = std::dynamic_pointer_cast<BaseNode>(*iter);
            if (node.get() == 0)
                {
                    continue;
                }

            std::shared_ptr<RigidBody> body = GetJointChildBody(node);
            if (body.get() != 0)
                {
                    return body;
                }
...
bool RosImporter::ReadUniversal(std::shared_ptr<BaseNode> parent, TiXmlElement* element)
{
    RosContextScope scope(this);
    GetContext().movable = true;
    ...
    std::shared_ptr<Transform> contextTransform = GetContextTransform(parent, anchorTrans);

    // joint
    std::shared_ptr<UniversalJoint> universal = std::dynamic_pointer_cast<UniversalJoint>
    ...
    std::shared_ptr<RigidBody> body1 = GetJointParentBody();
    std::shared_ptr<RigidBody> body2 = GetJointContext().body;
...
bool RosImporter::ReadHinge(std::shared_ptr<BaseNode> parent, TiXmlElement* element)
{
    RosContextScope scope(this);
    GetContext().movable = true;
    RosJointScope jointScope(this);
    ...
    // joint
    std::shared_ptr<HingeJoint> hinge = std::dynamic_pointer_cast<HingeJoint>
...
bool RosImporter::ReadSlider(std::shared_ptr<BaseNode> parent, TiXmlElement* element)
{
    RosContextScope scope(this);
    GetContext().movable = true;
    RosJointScope jointScope(this);
    ...
    // joint
    std::shared_ptr<SliderJoint> slider = std::dynamic_pointer_cast<SliderJoint>
...
bool RosImporter::ReadMacro(std::shared_ptr<BaseNode> parent, TiXmlElement* element)
{
    string name;
    if (! ReadAttribute(element, RA_NAME, name))
        {
    ...
    std::shared_ptr<TiXmlElement> macro(new TiXmlElement(*element));
...
bool RosImporter::ReadUse(std::shared_ptr<BaseNode> parent, TiXmlElement* element)
{
    string name;
    string instance;
    Trans trans;
    ...
    std::shared_ptr<TiXmlElement> tree = (*iter).second;
...
bool RosImporter::ReadComplexShape(std::shared_ptr<BaseNode> parent, TiXmlElement* element)
{
    std::shared_ptr<GeometryServer> geometryServer = std::dynamic_pointer_cast<GeometryServer>
    ...
    std::shared_ptr<StaticMesh> staticMesh = std::dynamic_pointer_cast<StaticMesh>
...
bool RosImporter::ReadGraphicalRep(TiXmlElement* element, std::shared_ptr<TriMesh> mesh, const Appearance& appear)
{
...
void RosImporter::BuildTriMesh(std::shared_ptr<TriMesh> mesh, TVertexList& vertexList,
...
bool RosImporter::ReadPhysicalRep(std::shared_ptr<BaseNode> parent, TiXmlElement* element)
{
    string name = S_UNNAMED;
    ReadAttribute(element, RA_NAME, name, true);
...
bool RosImporter::ReadSimpleBox(std::shared_ptr<oxygen::BaseNode> parent, TiXmlElement* element)
{
    string name;
    double length;
    double width;
    ...
    std::shared_ptr<RigidBody> body = GetContextBody(contextTransform);
    if (body.get() != 0)
        {
    ...
    if (physical.canCollide)
        {
            // geometry
            std::shared_ptr<TransformCollider> transCollider
...
bool RosImporter::ReadSimpleSphere(std::shared_ptr<oxygen::BaseNode> parent, TiXmlElement* element)
{
    string name;
    double radius;
    Trans trans;
...
bool RosImporter::ReadSimpleCapsule(std::shared_ptr<oxygen::BaseNode> parent, TiXmlElement* element)
{
    string name;
    double radius;
    double height;
...
void RosImporter::Attach(std::shared_ptr<Joint> joint, std::shared_ptr<RigidBody> body1, std::shared_ptr<RigidBody> body2,
...
void RosImporter::AttachJoint(const JointAttach& ja)
{
    ...
    std::shared_ptr<HingeJoint> hinge = std::dynamic_pointer_cast<HingeJoint>(ja.joint);
    if (hinge.get() != 0)
        {
    ...
    std::shared_ptr<UniversalJoint> universal = std::dynamic_pointer_cast<UniversalJoint>(ja.joint);
    if (universal.get() != 0)
        {
    ...
    std::shared_ptr<SliderJoint> slider = std::dynamic_pointer_cast<SliderJoint>(ja.joint);
    if (slider.get() != 0)
        {
