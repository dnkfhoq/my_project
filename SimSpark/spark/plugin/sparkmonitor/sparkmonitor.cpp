...
void SparkMonitor::DescribeLight(stringstream& ss, std::shared_ptr<Light> light)
{
    if (! mFullState)
        {
...
void SparkMonitor::DescribeTransform(stringstream& ss, NodeCache& entry, std::shared_ptr<Transform> transform)
{
    if (mFullState)
        {
            ss << "(nd TRF";
...
void SparkMonitor::DescribeMesh(stringstream& ss, std::shared_ptr<StaticMesh> mesh)
{
...
SparkMonitor::NodeCache* SparkMonitor::LookupNode(std::shared_ptr<BaseNode> node)
{
    if (node.get() == 0)
        {
            assert(false);
...
bool SparkMonitor::DescribeNode(stringstream& ss, std::shared_ptr<BaseNode> node)
{
...
void SparkMonitor::DescribeScene(stringstream& ss, std::shared_ptr<BaseNode> node)
{
