...
bool HingeEffector::Realize(std::shared_ptr<ActionObject> action)
{
    if (mJoint.get() == 0)
        {
            return false;
    ...
    std::shared_ptr<HingeAction> hingeAction =
        std::dynamic_pointer_cast<HingeAction>(action);
...
std::shared_ptr<ActionObject> HingeEffector::GetActionObject(const Predicate& predicate)
{
    for(;;)
        {
            if (mJoint.get() == 0)
            ...
            return std::shared_ptr<HingeAction>(new HingeAction(GetPredicate(),velocity));
        }
